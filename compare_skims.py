...
def compare_skims(path1, path2):
    ...
    try:
        ...
        if missing_in_2:
            logger.warning(
                f"Matrices present in '{os.path.basename(path1)}' but not in '{os.path.basename(path2)}': {missing_in_2}"
            )
        if missing_in_1:
            logger.warning(
                f"Matrices present in '{os.path.basename(path2)}' but not in '{os.path.basename(path1)}': {missing_in_1}"
            )
        ...
        for table_name in all_tables:
            ...
            if table_name in tables1:
                try:
                    ...
                    stats1 = {
                        "exists": True,
                        "error": str(e),
                    }  # Mark as exists but error happened
            ...
            if stats1["exists"]:
                if "error" in stats1:
                    ...
                    logger.info(
                        f"    NaNs: {stats1['nan_count']} ({stats1['nan_percent']:.2f}%)"
                    )
                    logger.info(
                        f"    Zeros: {stats1['zero_count']} ({stats1['zero_percent']:.2f}%)"
                    )
                    logger.info(
                        f"    Valid: {stats1['valid_count']} ({stats1['valid_percent']:.2f}%)"
                    )
            ...
            if stats2["exists"]:
                if "error" in stats2:
                    ...
                    logger.info(
                        f"    NaNs: {stats2['nan_count']} ({stats2['nan_percent']:.2f}%)"
                    )
                    logger.info(
                        f"    Zeros: {stats2['zero_count']} ({stats2['zero_percent']:.2f}%)"
                    )
                    logger.info(
                        f"    Valid: {stats2['valid_count']} ({stats2['valid_percent']:.2f}%)"
                    )
            ...
            if path and period and measure not in ["TRIPS", "FAILURES"]:
                ...
                feasibility_measures = [
                    "TOTIVT",
                    "IVT",
                    "TIME",
                    "DIST",
                ]  # Prioritize time/dist measures
                ...
                if feasibility_matrix_name:
                    logger.info(
                        f"  Using '{feasibility_matrix_name}' to determine feasible ODs."
                    )
                    ...
                    if feasibility_matrix_name in tables1:
                        try:
                            feasibility_data1 = np.array(
                                skims1[feasibility_matrix_name]
                            )
                        except Exception as e:
                            logger.error(
                    ...
                    if feasibility_matrix_name in tables2:
                        try:
                            feasibility_data2 = np.array(
                                skims2[feasibility_matrix_name]
                            )
                        except Exception as e:
                            logger.error(
                    ...
                    if feasibility_data1 is not None and feasibility_data2 is not None:
                        if feasibility_data1.shape != feasibility_data2.shape:
                            ...
                            mask1 = (~np.isnan(feasibility_data1)) & (
                                feasibility_data1 > 0
                            )
                            mask2 = (~np.isnan(feasibility_data2)) & (
                                feasibility_data2 > 0
                            )
                        ...
                        logger.debug(
                            f"  Feasibility matrix {feasibility_matrix_name} not found in either file."
                        )
                        shape_match = False  # Cannot do full comparison
                    ...
                    if mask1 is not None and mask2 is not None and shape_match:
                        ...
                        logger.info(
                            f"  Feasible ODs (where {feasibility_matrix_name} > 0):"
                        )
                        logger.info(f"    File 1: {feasible_count1}")
                        logger.info(f"    File 2: {feasible_count2}")
                        logger.info(
                            f"    Difference (File 2 - File 1): {feasible_diff}"
                        )
                        ...
                        if feasible_count1 > 0 or feasible_count2 > 0:
                            ...
                            logger.info(
                                f"  OD Category Counts (based on {feasibility_matrix_name} > 0):"
                            )
                            ...
                            if (
                                ...
                                if stats1["shape"] == stats2["shape"]:
                                    logger.info(
                                        f"  Segmented Averages for {table_name}:"
                                    )

                                    def safe_mean(arr, mask):
                                        # Calculate mean only if mask is not empty and filtered data is not all NaN
                                        filtered_data = arr[mask]
                                        if filtered_data.size > 0 and not np.all(
                                            np.isnan(filtered_data)
                                        ):
                                            return np.nanmean(
                                                filtered_data
                                            )  # Use nanmean to ignore NaNs in selected slice
                                        return (
                                            np.nan
                                        )  # Return NaN if no data or all NaN
                                    ...
                                    mean2_f1only = safe_mean(
                                        data2, mask_file1_only
                                    )  # Value in file 2 should ideally be 0 or NaN if not feasible
                                    logger.info(
                                        f"    Feasible in File 1 only ({f1_only_count} ODs): File 1 Avg={mean1_f1only:.4f}, File 2 Avg={mean2_f1only:.4f}"
                                    )

                                    # File 2 only feasible
                                    mean1_f2only = safe_mean(
                                        data1, mask_file2_only
                                    )  # Value in file 1 should ideally be 0 or NaN if not feasible
                ...
                if stats1["shape"] != stats2["shape"]:
                    ...
                    if nan_diff > 1.0:  # Threshold for warning
                        ...
                        comparison_summary.append(
                            f"{table_name}: Overall NaN Diff={nan_diff:.2f}%"
                        )
                    ...
                    if zero_diff > 1.0:  # Threshold for warning
                        ...
                        comparison_summary.append(
                            f"{table_name}: Overall Zero Diff={zero_diff:.2f}%"
                        )
                    ...
                    if stats1["valid_count"] > 0 and stats2["valid_count"] > 0:
                        ...
                        for stat_name in ["mean", "median", "min", "max", "std"]:
                            ...
                            significant_abs_diff = abs(val1 - val2) > 0.01 and (
                                abs(val1) > 1e-9 or abs(val2) > 1e-9
                            )  # Reduced threshold for absolute diff
                            ...
                            if significant_ratio_diff or significant_abs_diff:
                                ...
                                comp_notes.append(
                                    f"Overall {stat_name} Ratio={ratio:.2f}"
                                )

                        if comp_notes:
                            comparison_summary.append(
                                f"{table_name}: {', '.join(comp_notes)}"
                            )
                        ...
                        comparison_summary.append(
                            f"{table_name}: Overall NO VALID DATA IN FILE 1"
                        )
                    elif stats1["valid_count"] > 0 and stats2["valid_count"] == 0:
                        logger.warning(
                        ...
                        comparison_summary.append(
                            f"{table_name}: Overall NO VALID DATA IN FILE 2"
                        )
                    elif stats1["valid_count"] == 0 and stats2["valid_count"] == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("path1", help="Path to the first OMX skim file.")
    parser.add_argument("path2", help="Path to the second OMX skim file.")

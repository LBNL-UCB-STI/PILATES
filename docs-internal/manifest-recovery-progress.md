...
| 6. Land-use opt-out | done | 3688d87525241e12c9d8c13f0e8da26daf3bd1c5 | `rtk /Users/zaneedell/miniforge3/envs/PILATES/bin/python -m pytest tests/test_architecture_guardrails.py tests/test_stage_contracts.py tests/test_land_use_manifest_persistence.py -q` -> 81 passed, 3 warnings; `rtk /Users/zaneedell/miniforge3/envs/PILATES/bin/python -m ty check pilates/workflows/stages/land_use.py` -> all checks passed; `rtk /Users/zaneedell/miniforge3/envs/PILATES/bin/python -m pytest tests/test_architecture_guardrails.py tests/test_postprocessing_manifest_persistence.py tests/test_land_use_manifest_persistence.py tests/test_stage_contracts.py -q` -> 83 passed, 3 warnings; `rtk /Users/zaneedell/miniforge3/envs/PILATES/bin/python -m ty check tests/test_land_use_manifest_persistence.py` -> all checks passed | spec: passed after fix; quality: passed | Land use uses policy-built manifest config; disabled path runs all UrbanSim steps, writes no manifest YAML, and preserves handoff mapping from sibling baseline couplers. |
| 7. Supply-demand guard and follow-up | done | 344b73a597fe366551cb5088782b627ef98c929d | Worker: `rtk /Users/zaneedell/miniforge3/envs/PILATES/bin/python -m pytest tests/test_recovery_store.py tests/test_restart_stage_boundary_matrix.py tests/test_manifest_cache_parity.py -q` -> 41 passed, 1 warning before controller P1 fix; controller: `rtk /Users/zaneedell/miniforge3/envs/PILATES/bin/python -m ty check pilates/workflows/recovery.py tests/test_recovery_store.py` -> all checks passed; controller pytest rerun blocked before collection by adjacent editable Consist import failure: missing `consist.core.output_sets`. | spec: passed after P1 fix; quality: passed after P1 fix | Added temporary guard for compatibility-backend-track supply-demand manifests plus deletion follow-up doc. |
| 8. Retire direct ManifestConfig usage in migrated stages | done | e0682923575019ee2437d6253c01033e1c6f09d0 | `rtk /Users/zaneedell/miniforge3/envs/PILATES/bin/python -m pytest tests/test_architecture_guardrails.py -q` -> 10 passed; `rtk /Users/zaneedell/miniforge3/envs/PILATES/bin/python -m ty check tests/test_architecture_guardrails.py` -> all checks passed; `git diff --check` -> passed | spec: passed; quality: P3 fixed | Explicitly guard postprocessing and land-use as policy-managed migrated stages; supply-demand and vehicle ownership remain allowlisted. |
...
| Task 7 spec review | fixed | Initial review had no actionable findings before quality P1 fix. | Re-review passed after live-path guard expansion. |
| Task 7 quality review | fixed | Live supply-demand stage only failed fast for `supply_demand`, leaving `activity_demand`/`activity_demand_compile` disabled-stage names able to bypass the guard. Follow-up doc was untracked. | Added `ensure_supply_demand_manifest_disablement_allowed(...)`, call it at supply-demand stage entry, added live-path regression, and will stage the follow-up doc explicitly. |
| Task 8 spec review | fixed | No actionable findings. | No patch needed. |
| Task 8 quality review | fixed | Task 8 ledger row left completed verification evidence as pending. | Recorded architecture guard, type check, and diff-check evidence. |

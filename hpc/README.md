...
By default this requests `230G` on `lr7`.
...
- For default `lr7` (230G job memory), default is `180g`.
...
2. Install dependencies from the requirements file when needed.
3. Cache the requirements hash in `$VENV_PATH/.last_requirements_hash` to skip unnecessary reinstalls.
4. Install `consist` from local source if available, otherwise from PyPI when needed.
...
```bash
PILATES_LOG_DIR=/path/to/pilates_logs ./hpc/job_runner.sh -c settings-seattle-newconfig-hpc.yaml
```
...
```bash
python3 scripts/migrate_config.py <old> <tmp_migrated>.yaml --no-validate
```

and uses the migrated file for that job only.

...
2. Otherwise, install from PyPI package name in `CONSIST_PYPI_PACKAGE` or fall back to `consist==0.3.1`.
...
```bash
CONSIST_PYPI_PACKAGE=consist==0.3.1 ./hpc/job_runner.sh -c scenarios/seattle/settings-seattle-consist-hpc.yaml -a <slurm_account>
```

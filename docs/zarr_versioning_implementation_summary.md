# Data Versioning & Analysis Architecture
...
This document describes the architecture for versioning PILATES simulation data artifacts (e.g., Zarr skims, NetCDF outputs). The architecture provides **robust, immutable storage** for data provenance and reproducibility, combined with a **powerful, flexible analysis layer** for ergonomic, cross-run comparisons.
...
The architecture decouples physical storage from logical analysis, while being flexible enough to handle various data formats like Zarr and NetCDF.
...
-   **Path Scheme**: `s3://<bucket>/data_archives/{snapshot_id}/artifact.zarr` (or `artifact.nc`). The `artifact_path` column in the database specifies the exact location.
...
-   **Component**: A new `SnapshotAnalysisManager` class serves as the query engine.
-   **Workflow**:
...
**Location**: `pilates/utils/snapshot_analysis.py`

This class is the primary tool for post-run analysis.
...
2.  **Analysis View Optimization**: For very large queries (spanning terabytes of skim data), the `SnapshotAnalysisManager` can be optimized to use distributed computing backends like Dask.
...
This two-layer architecture provides a robust and simple foundation for data versioning while enabling a highly sophisticated and flexible analysis workflow. It addresses the key limitations of the previous plan and is well-suited for a scalable, multi-run simulation environment.

...
-   **`land_use.csv` Creation:** The preprocessor calls `load_canonical_zones` to get the sorted zone data. It then aggregates population, employment, and other data, joining it to this sorted frame. When `land_use.csv` is written, it naturally preserves the canonical sorted order.
...
-   **Action:** It compares the `original_zone_ids` attribute from the new BEAM Zarr skims against the canonical order loaded via `load_canonical_zones`.
-   **Safeguard:** If the orders do not match exactly, it raises a `ValueError` and stops the simulation, preventing misaligned data from being used.
...
-   **Verification is Key:** The `verify_skim_zone_order` check in the BEAM postprocessor acts as a critical safeguard to catch any unexpected changes or bugs in BEAM's output ordering.

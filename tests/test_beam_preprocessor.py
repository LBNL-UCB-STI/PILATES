...
def mock_beam_shapefile(mock_workspace):
    ...
    shapefile_dir = os.path.join(mock_workspace.get_beam_mutable_data_dir(), "seattle", "shape")
    ...
    polygons = [Polygon([(i, i), (i+1, i), (i+1, i+1), (i, i+1)]) for i in range(len(scrambled_geoids))]
...
class TestBeamPreprocessor:
    """Tests for the BEAM Preprocessor, focusing on zone order alignment."""
    ...
    def test_shapefile_sorting_and_indexing(
        ...
        sorted_gdf = gpd.read_file(mock_beam_shapefile)
        ...
        assert final_order == CANONICAL_GEOID_ORDER, "Shapefile GEOID order does not match canonical order after sorting."

        # 2. Verify provenance tracking
        # 2a. Check that the original file was recorded as an input
        provenance_tracker.record_input_file.assert_called_once_with(
            "beam_preprocessor",
            mock_beam_shapefile,
        ...
        # 2b. Check that the modified file was recorded as an output
        provenance_tracker.record_output_file_with_inputs.assert_called_once()
        call_args = provenance_tracker.record_output_file_with_inputs.call_args
        assert call_args[0][1] == mock_beam_shapefile # path
        assert call_args[1]['short_name'] == "beam_zone_shapefile_sorted"
        assert call_args[1]['model_run_id'] == model_run_hash

...
def create_dummy_run_info(
    tmp_path, model_name, year, iteration, sub_stage_progress, output_records=None
):
    run_id = "test_run_id"
    ...
    if sub_stage_progress in ["runner", "postprocessor"]:
        ...
        preprocessor_run_id = (
            f"{preprocessor_model_name}_{datetime.now().strftime('%Y%m%d')}_abcde123"
        )
        ...
        tracker.run_info.file_records[preprocessor_output_file_hash] = (
            preprocessor_output_record
        )
    ...
    if sub_stage_progress == "postprocessor":
        ...
        runner_run_id = (
            f"{runner_model_name}_{datetime.now().strftime('%Y%m%d')}_fghij456"
        )
...
def setup_test_environment(tmp_path):
    ...
    with patch("workflow_state.WorkflowState.from_settings") as mock_from_settings:
        ...
        mock_state.set_sub_stage_progress.side_effect = lambda x: setattr(
            mock_state, "sub_stage_progress", x
        )
...
def test_fresh_run_executes_all_sub_stages(setup_test_environment):
    ...
    with patch(
        "pilates.urbansim.preprocessor.UrbansimPreprocessor._preprocess"
    ) as mock_pre_preprocess:
        with patch("pilates.urbansim.runner.UrbansimRunner._run") as mock_run_run:
            with patch(
                "pilates.urbansim.postprocessor.UrbansimPostprocessor._postprocess"
            ) as mock_post_postprocess:
                ...
                preprocessor_instance = factory.get_preprocessor(
                    "urbansim",
                    mock_state,
                    MagicMock(),
                    major_stage=WorkflowState.Stage.land_use,
                )
                runner_instance = factory.get_runner(
                    "urbansim",
                    mock_state,
                    MagicMock(),
                    major_stage=WorkflowState.Stage.land_use,
                )
                postprocessor_instance = factory.get_postprocessor(
                    "urbansim",
                    mock_state,
                    MagicMock(),
                    major_stage=WorkflowState.Stage.land_use,
                ...
                assert (
                    mock_state.sub_stage_progress == "postprocessor"
                )  # Last one to be set
...
def test_resume_from_runner_failure_skips_preprocessor(setup_test_environment):
    ...
    tracker = create_dummy_run_info(
        tmp_path,
        "urbansim",
        mock_state.current_year,
        mock_state.current_inner_iter,
        "runner",
    )
    ...
    with patch(
        ...
        with patch("pilates.urbansim.runner.UrbansimRunner._run") as mock_run_run:
            with patch(
                ...
                with patch(
                    "pilates.utils.provenance.FileProvenanceTracker._initialize_run_info",
                    return_value=tracker.run_info,
                ):
                    # Instantiate the ModelFactory
                    factory = ModelFactory()
                    ...
                    assert (
                        input_data.all_records()[0].unique_id
                        == "preprocessor_output_hash"
                    )
...
def test_resume_from_postprocessor_failure_skips_preprocessor_and_runner(

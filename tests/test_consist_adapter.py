...
def consist_tracker(tmp_workspace):
    """Create a ConsistProvenanceTracker instance."""
...
def test_consist_tracker_instantiation(tmp_workspace):
    ...
    assert tracker.workspace_root == str(tmp_workspace)
...
def test_consist_tracker_instantiation_with_folder_name(tmp_workspace):
    ...
    assert tracker.workspace_root == str(tmp_workspace / "subfolder")
...
def test_consist_tracker_instantiation_with_all_parameters(tmp_workspace):
    ...
    assert tracker.workspace_root == str(tmp_workspace / "test_run")
...
def test_workspace_root_with_folder(tmp_workspace):
    ...
    assert tracker.workspace_root == expected
...
def test_none_output_path_handling():
    ...
    assert tracker.workspace_root is None
...
def test_tracker_initialization_with_mounts(tmp_workspace):
    ...
    assert tracker._tracker is not None
    # Mounts should be passed to Consist tracker
...
def test_start_and_complete_model_run_success(consist_tracker):
    ...
    # Start a model run (note: description causes duplicate kwarg issue in adapter)
    try:
        ...
        # Adapter has issue with description kwarg duplication
...
def test_start_and_complete_model_run_failed(consist_tracker):
    ...
    # Start a model run (note: avoid description to prevent kwarg duplication)
    try:
        model_run_id = consist_tracker.start_model_run(
            model="beam",
            year=2021,
...
def test_record_h5_input_container(tmp_workspace):
    ...
    try:
        ...
        if h5_record is not None:
            ...
            assert str(h5_file) in tracker._h5_containers
...
def test_initialize_from_settings_basic(tmp_workspace):
    ...
    assert tracker.run_info.settings_hash is not None

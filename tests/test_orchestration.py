...
SETTINGS_FILES = {
    "landuse_off_replanning_on": os.path.join(
        os.path.dirname(RUN_SCRIPT), "settings_test_landuse_off_replanning_on.yaml"
    ),
    "landuse_on_replanning_on": os.path.join(
        os.path.dirname(RUN_SCRIPT), "settings_test_landuse_on_replanning_on.yaml"
    ),
    "landuse_on_replanning_off": os.path.join(
        os.path.dirname(RUN_SCRIPT), "settings_test_landuse_on_replanning_off.yaml"
    ),
}
...
def test_orchestration_with_stubs(config_name, settings_file):
    ...
    if settings.get("land_use_enabled"):
        ...
        enabled_models.add(
            "atlas"
        )  # Atlas might run even if land_use_enabled is false depending on config
    if settings.get("replanning_enabled"):
        enabled_models.add("activitysim")
        enabled_models.add(
            "beam"
        )  # BEAM might run even if replanning_enabled is false depending on config
    ...
    assert (
        result.returncode == 0
    ), f"run.py failed with settings {settings_file}. Stderr: {result.stderr}"
    ...
    full_output_dir = os.path.abspath(
        output_dir_setting
    )  # Assuming output_dir is relative to workspace root

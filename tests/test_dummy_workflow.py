...
from pilates.generic.records import H5FileRecord, H5TableRecord, FileRecord, ModelRunInfo, RecordStore
...
class DummyModelAPreprocessor(GenericPreprocessor):
    ...
    def copy_data_to_mutable_location(self, settings: dict, output_dir: str) -> Tuple[RecordStore, RecordStore]:
        """
        ...
        input_h5_file_record.table_record_ids = [input_h5_table1_record.unique_id, input_h5_table2_record.unique_id]


        self.provenance_tracker.record_input_file(model=self.model_name, file_path=input_csv_record.file_path, description=input_csv_record.description, short_name=input_csv_record.short_name, context=self.state)
        self.provenance_tracker.record_input_file(model=self.model_name, file_path=input_h5_file_record.file_path, description=input_h5_file_record.description, short_name=input_h5_file_record.short_name, context=self.state)
        self.provenance_tracker.record_input_file(model=self.model_name, file_path=input_h5_table1_record.file_path, description=input_h5_table1_record.description, short_name=input_h5_table1_record.short_name, context=self.state)
        self.provenance_tracker.record_input_file(model=self.model_name, file_path=input_h5_table2_record.file_path, description=input_h5_table2_record.description, short_name=input_h5_table2_record.short_name, context=self.state)


        # Record provenance for copied files (outputs of this copy operation)
        output_csv_path = os.path.join(output_dir, 'data.csv')
        output_csv_record = FileRecord(file_path=output_csv_path, short_name='data.csv', unique_id=make_unique_id(output_csv_path))
        output_h5_path = os.path.join(output_dir, 'data.h5')
        output_h5_file_record = H5FileRecord(file_path=output_h5_path, short_name='data.h5', unique_id=make_unique_id(output_h5_path))
        ...
        output_h5_file_record.table_record_ids = [output_h5_table1_record.unique_id, output_h5_table2_record.unique_id]

        self.provenance_tracker.record_output_file(model=self.model_name, file_path=output_csv_record.file_path, description=output_csv_record.description, short_name=output_csv_record.short_name, context=self.state)
        self.provenance_tracker.record_output_file(model=self.model_name, file_path=output_h5_file_record.file_path, description=output_h5_file_record.description, short_name=output_h5_file_record.short_name, context=self.state)
        self.provenance_tracker.record_output_file(model=self.model_name, file_path=output_h5_table1_record.file_path, description=output_h5_table1_record.description, short_name=output_h5_table1_record.short_name, context=self.state)
        self.provenance_tracker.record_output_file(model=self.model_name, file_path=output_h5_table2_record.file_path, description=output_h5_table2_record.description, short_name=output_h5_table2_record.short_name, context=self.state)


        return RecordStore(recordList=[input_csv_record, input_h5_file_record, input_h5_table1_record, input_h5_table2_record]), \
               RecordStore(recordList=[output_csv_record, output_h5_file_record, output_h5_table1_record, output_h5_table2_record])

    def _preprocess(self, workspace: Workspace, previous_records: RecordStore = RecordStore()) -> RecordStore:
...
class DummyModelARunner(GenericRunner):
    ...
    def _run(self, store: RecordStore, workspace: Workspace) -> RecordStore:
        ...
        if not csv_record:
            raise ValueError("data.csv record not found in RecordStore for Model A Runner.")
        if not h5_record:
            raise ValueError("data.h5 record not found in RecordStore for Model A Runner.")
        ...
        with h5py.File(output_h5_path, 'w') as f_out:
            f_out.create_dataset('table1_modified', data=df_table1_modified.to_records(index=False))
        ...
        self.provenance_tracker.record_output_file(model=self.model_name, file_path=output_h5_file_record.file_path, description=output_h5_file_record.description, short_name=output_h5_file_record.short_name, context=self.state)
        self.provenance_tracker.record_output_file(model=self.model_name, file_path=output_h5_table_record.file_path, description=output_h5_table_record.description, short_name=output_h5_table_record.short_name, context=self.state)


        return RecordStore(
            recordList=[
                FileRecord(file_path=output_csv_path, short_name=f'model_a_output_{year}.csv', unique_id=make_unique_id(output_csv_path)),
...
class DummyModelAPostprocessor(GenericPostprocessor):
    ...
    def _postprocess(self, raw_outputs: RecordStore, workspace: Workspace, runInfo=None, model_run_hash=None) -> RecordStore:
        output_dir = workspace.output_dir
        output_csv_record = get_record_by_short_name(raw_outputs, f'model_a_output_{self.state.current_year}.csv')
        output_h5_record = get_record_by_short_name(raw_outputs, f'model_a_output_{self.state.current_year}.h5')

        if not output_csv_record:
            raise ValueError("model_a_output_*.csv record not found in raw_outputs for Model A Postprocessor.")
        if not output_h5_record:
            raise ValueError("model_a_output_*.h5 record not found in raw_outputs for Model A Postprocessor.")
        ...
        with h5py.File(final_h5_path, 'w') as f_out:
            f_out.create_dataset('table1_final', data=df_table_final.to_records(index=False))
        ...
        self.provenance_tracker.record_output_file(model=self.model_name, file_path=final_h5_file_record.file_path, description=final_h5_file_record.description, short_name=final_h5_file_record.short_name, context=self.state)
        self.provenance_tracker.record_output_file(model=self.model_name, file_path=final_h5_table_record.file_path, description=final_h5_table_record.description, short_name=final_h5_table_record.short_name, context=self.state)

        return RecordStore(
            recordList=[
                FileRecord(file_path=final_csv_path, short_name=f'model_a_final_output_{year}.csv', unique_id=make_unique_id(final_csv_path)),
...
class DummyModelBPreprocessor(GenericPreprocessor):
    ...
    def _preprocess(self, workspace: Workspace, previous_records: RecordStore = RecordStore()) -> RecordStore:
        output_dir = workspace.output_dir
        model_a_final_csv_record = get_record_by_short_name(previous_records, f'model_a_final_output_{self.state.current_year}.csv')
        model_a_final_h5_record = get_record_by_short_name(previous_records, f'model_a_final_output_{self.state.current_year}.h5')

        if not model_a_final_csv_record:
            raise ValueError("model_a_final_output_*.csv record not found in previous_records for Model B Preprocessor.")
        if not model_a_final_h5_record:
            raise ValueError("model_a_final_output_*.h5 record not found in previous_records for Model B Preprocessor.")
...
class DummyModelBRunner(GenericRunner):
    ...
    def _run(self, store: RecordStore, workspace: Workspace) -> RecordStore:
        output_dir = workspace.output_dir
        csv_record = get_record_by_short_name(store, f'model_a_final_output_{self.state.current_year}.csv')
        h5_record = get_record_by_short_name(store, f'model_a_final_output_{self.state.current_year}.h5')

        if not csv_record:
            raise ValueError("model_a_final_output_*.csv record not found in RecordStore for Model B Runner.")
        if not h5_record:
            raise ValueError("model_a_final_output_*.h5 record not found in RecordStore for Model B Runner.")
        ...
        with h5py.File(output_h5_path, 'w') as f_out:
            f_out.create_dataset('table_b_modified', data=df_table_modified.to_records(index=False))
        ...
        return RecordStore(
            recordList=[
                FileRecord(file_path=output_csv_path, short_name=f'model_b_output_{year}.csv', unique_id=make_unique_id(output_csv_path)),
                output_h5_file_record,
                output_h5_table_record
            ]
        ), None
...
class DummyModelBPostprocessor(GenericPostprocessor):
    ...
    def _postprocess(self, raw_outputs: RecordStore, workspace: Workspace, runInfo=None, model_run_hash=None) -> RecordStore:
        output_dir = workspace.output_dir
        output_csv_record = get_record_by_short_name(raw_outputs, f'model_b_output_{self.state.current_year}.csv')
        output_h5_record = get_record_by_short_name(raw_outputs, f'model_b_output_{self.state.current_year}.h5')

        if not output_csv_record:
            raise ValueError("model_b_output_*.csv record not found in raw_outputs for Model B Postprocessor.")
        if not output_h5_record:
            raise ValueError("model_b_output_*.h5 record not found in raw_outputs for Model B Postprocessor.")
        ...
        final_h5_summary_path = os.path.join(output_dir, f'model_b_final_output_summary_{year}.txt')
        ...
        return RecordStore(
            recordList=[
                FileRecord(file_path=final_txt_path, short_name=f'model_b_final_output_{year}.txt', unique_id=make_unique_id(final_txt_path)),
                FileRecord(file_path=final_h5_summary_path, short_name=f'model_b_final_output_summary_{year}.txt', unique_id=make_unique_id(final_h5_summary_path))
            ]
        )
...
class TestDummyWorkflow:
    ...
    def setup_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            provenance_tracker = OpenLineageTracker(run_id="test_run", output_path=str(workflow_output_dir))
    ...
    def test_single_year_workflow(self, setup_workflow):
        ...
        model_a_preprocessor = DummyModelAPreprocessor("ModelA", model_a_config, provenance_tracker, workflow_state)
        model_a_runner = DummyModelARunner("ModelA", model_a_config, provenance_tracker, workflow_state)
        model_a_postprocessor = DummyModelAPostprocessor("ModelA", model_a_config, provenance_tracker, workflow_state)

        # Call copy_data_to_mutable_location first
        _, mutable_location_records_a = model_a_preprocessor.copy_data_to_mutable_location(model_a_config, str(workflow_output_dir))

        # Call public methods, passing the records from mutable_location
        preprocessor_a_output_records = model_a_preprocessor.preprocess(workspace, previous_records=mutable_location_records_a)
        runner_a_output_records, _ = model_a_runner.run(preprocessor_a_output_records, workspace)
        postprocessor_a_output_records = model_a_postprocessor.postprocess(runner_a_output_records, workspace)
        ...
        assert 'a_doubled' in df_a_final.columns, "Model A output should have 'a_doubled' column"
        assert (df_a_final['a_doubled'] > 10).all(), "Model A output should only contain rows where a_doubled > 10"
        ...
        model_b_preprocessor = DummyModelBPreprocessor("ModelB", model_b_config, provenance_tracker, workflow_state)
        model_b_runner = DummyModelBRunner("ModelB", model_b_config, provenance_tracker, workflow_state)
        model_b_postprocessor = DummyModelBPostprocessor("ModelB", model_b_config, provenance_tracker, workflow_state)

        # Call copy_data_to_mutable_location for Model B (returns empty RecordStores)
        _, mutable_location_records_b = model_b_preprocessor.copy_data_to_mutable_location(model_b_config, str(workflow_output_dir))

        # Call public methods, passing output from Model A's postprocessor as input
        preprocessor_b_output_records = model_b_preprocessor.preprocess(workspace, previous_records=postprocessor_a_output_records + mutable_location_records_b)
        runner_b_output_records, _ = model_b_runner.run(preprocessor_b_output_records, workspace)
        postprocessor_b_output_records = model_b_postprocessor.postprocess(runner_b_output_records, workspace)
        ...
        with open(model_b_txt_path, 'r') as f:
            ...
            assert 'Row count from Model B CSV:' in row_count_content, "Model B output should contain row count"

        with open(model_b_summary_path, 'r') as f:
            sum_content = f.read()
            assert 'Total sum from Model B H5:' in sum_content, "Model B summary should contain H5 sum"
        ...
        file_record_count = conn.execute("SELECT COUNT(*) FROM file_records").fetchone()[0]
        assert file_record_count > 0, f"No file records found in database (expected records from CSV and H5 files)"
        ...
        records = conn.execute("SELECT short_name, description FROM file_records ORDER BY short_name").fetchall()
        ...
        print(f"  Sample records: {short_names[:5] if len(short_names) > 5 else short_names}")

        conn.close()

        print(f"✓ Workflow output in: {workflow_output_dir}")
        print(f"✓ Provenance DB in: {db_path}")
        print(f"✓ Database integration complete: {file_record_count} file records uploaded")

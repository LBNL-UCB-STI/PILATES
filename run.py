...
def get_usim_docker_vols(settings, output_dir=None):
    ...
    if output_dir is None:
        output_dir = settings['usim_local_data_input_folder'] # This seems wrong, should be mutable output dir
        logger.warning("get_usim_docker_vols called without output_dir, using usim_local_data_input_folder. Check logic.")
...
def get_atlas_docker_vols(settings, working_dir=None):
    if working_dir is None:
        ...
        atlas_host_output_folder = os.path.abspath(os.path.join(working_dir, settings['atlas_host_output_folder']))
...
def warm_start_activities(settings, state: WorkflowState, client):
    ...
    if activity_demand_model == 'polaris':
        ...
        additional_args = get_asim_additional_args(settings, asim_docker_vols, False) # No compile in warm start run

        # Record ActivitySim warm start run start
        asim_run_index = state.record_model_start(activity_demand_model, year=state.current_year, iteration=0)

        success = run_container(client, settings, activity_demand_image, asim_docker_vols, ws_asim_cmd, working_dir=asim_workdir, model_name=activity_demand_model, args=additional_args)

        # Record ActivitySim warm start run completion
        state.record_model_completion(asim_run_index, status="completed" if success else "failed")
        if not success:
        ...
        state.record_input_file(land_use_model, asim_warm_start_persons_path, description="ActivitySim warm start persons output for USim update")
        state.record_input_file(land_use_model, asim_warm_start_households_path, description="ActivitySim warm start households output for USim update")
        usim_input_store_path = os.path.join(state.full_path, settings['usim_local_mutable_data_folder'], usim_post.get_usim_datastore_fname(settings, io='input'))
        state.record_input_file(land_use_model, usim_input_store_path, description="UrbanSim input data before warm start update")


        asim_post.update_usim_inputs_after_warm_start(settings, usim_data_dir=os.path.join(state.full_path, settings['usim_local_mutable_data_folder']), warm_start_dir=os.path.join(state.full_path, settings['asim_local_output_folder']))

        # Record outputs from UrbanSim update postprocessing
        state.record_output_file(land_use_model, usim_input_store_path, year=state.current_year, description="UrbanSim input data after warm start update")
...
def forecast_land_use(settings, year, workflow_state: WorkflowState, client, container_manager):
    ...
    usim_run_index = workflow_state.record_model_start(land_use_model, year=workflow_state.forecast_year)

    run_land_use(settings, year, workflow_state, client)

    # Record UrbanSim run completion
    usim_output_store_name = usim_post.get_usim_datastore_fname(settings, io='output', year=workflow_state.forecast_year)
    output_dir = os.path.join(workflow_state.full_path, settings['usim_local_mutable_data_folder'])
    ...
    if os.path.exists(usim_datastore_fpath):
        ...
        workflow_state.record_output_file(land_use_model, usim_datastore_fpath, year=workflow_state.forecast_year, description="UrbanSim forecast output data")
    else:
        logger.critical(
...
def run_land_use(settings, year, workflow_state: WorkflowState, client):
    ...
    land_use_model, land_use_image = get_model_and_image(settings, "land_use_model")
    usim_docker_vols = get_usim_docker_vols(settings, output_dir) # Pass the mutable output_dir
    ...
    asim_output_dir = os.path.join(workflow_state.full_path, settings['asim_local_mutable_data_folder'])
    asim_skims_path = os.path.join(asim_output_dir, settings['skims_fname']) # Assuming skims are here
    workflow_state.record_input_file(land_use_model, asim_skims_path, description="ActivitySim skims for UrbanSim input")

    usim_pre.add_skims_to_model_data(settings, output_dir, asim_output_dir)

    usim_data_path = os.path.join(settings['usim_local_data_input_folder'], # This path seems inconsistent with mutable output_dir
    ...
    usim_input_in_run_dir = os.path.join(output_dir, usim_post.get_usim_datastore_fname(settings, io='input'))
    workflow_state.record_input_file(land_use_model, usim_input_in_run_dir, description="UrbanSim input data for forecast")


    if is_already_opened_in_write_mode(usim_data_path): # This check might need adjustment if using mutable copy
        logger.warning(
...
def run_atlas(settings, state: WorkflowState, client, warm_start_atlas, forecast=False, atlas_run_count=1):
    ...
    usim_output_store_name = usim_post.get_usim_datastore_fname(settings, io='output', year=state.forecast_year)
    usim_output_store_path = os.path.join(state.full_path, settings['usim_local_mutable_data_folder'], usim_output_store_name)
    state.record_input_file(vehicle_ownership_model, usim_output_store_path, description="UrbanSim output for Atlas input preparation")
    ...
    for yr_it in yrs:
        ...
        atlas_input_csv_dir = os.path.join(state.full_path, settings['atlas_host_mutable_input_folder'], f'atlas_inputs/year{yr_it}')
        if os.path.exists(atlas_input_csv_dir):
             for file in os.listdir(atlas_input_csv_dir):
                  if file.endswith('.csv'):
                       state.record_output_file(vehicle_ownership_model, os.path.join(atlas_input_csv_dir, file), year=yr_it, description="Atlas preprocessed input CSV")
    ...
    if beamac > 0:
        ...
        expected_beam_skims_path = os.path.join(beam_output_dir, settings['skims_fname'])
        state.record_input_file(vehicle_ownership_model, expected_beam_skims_path, description="BEAM skims for Atlas accessibility calculation")
        ...
        atlas_acc_output_path = os.path.join(state.full_path, settings['atlas_host_mutable_input_folder'], f'atlas_inputs/year{state.forecast_year}', 'accessibility.csv')
        state.record_output_file(vehicle_ownership_model, atlas_acc_output_path, year=state.forecast_year, description="Atlas accessibility output CSV")
    ...
    atlas_run_index = state.record_model_start(vehicle_ownership_model, year=yr, iteration=atlas_run_count)

    success = run_container(client, settings, atlas_image, atlas_docker_vols, atlas_cmd, working_dir='/', model_name=vehicle_ownership_model)

    # Record Atlas run completion
    state.record_model_completion(atlas_run_index, status="completed" if success else "failed")
    if not success:
    ...
    atlas_vehicle_output_file = os.path.join(atlas_output_path, f'vehicles_{yr}.csv') # Assuming CSV output before gz
    # Record Atlas output file (before it's potentially gzipped or moved)
    state.record_output_file(vehicle_ownership_model, atlas_vehicle_output_file, year=yr, description="Atlas raw vehicle ownership output CSV")

    # Record inputs to Atlas postprocessing (Atlas output CSV and UrbanSim output H5)
    state.record_input_file(vehicle_ownership_model, atlas_vehicle_output_file, description="Atlas vehicle ownership output for USim update")
    state.record_input_file(vehicle_ownership_model, usim_output_store_path, description="UrbanSim output data for Atlas update") # Re-using path from earlier
    ...
    state.record_output_file(vehicle_ownership_model, usim_output_store_path, year=state.forecast_year, description="UrbanSim output data after Atlas update")
    ...
    state.record_input_file(vehicle_ownership_model, atlas_vehicle_output_file, description="Atlas vehicle ownership output for BEAM vehicle input")
    state.record_input_file(vehicle_ownership_model, usim_output_store_path, description="UrbanSim output data for BEAM vehicle input") # Re-using path
    ...
    state.record_output_file(vehicle_ownership_model, beam_vehicles_path, year=yr, description="BEAM vehicles input file generated from Atlas/UrbanSim")
...
def run_atlas_auto(settings, state: WorkflowState, client, warm_start_atlas, forecast=False):
    ...
    if warm_start_atlas and os.path.exists(os.path.join(atlas_output_path, gz_fname)):
        ...
        state.record_output_file(settings.get('vehicle_ownership_model', 'atlas'), os.path.join(atlas_output_path, gz_fname), year=yr, description="Atlas gzipped vehicle ownership output (skipped run)")
        return
    ...
    while atlas_run_count <= 3 and not success:
        ...
        try:
            ...
            if os.path.exists(os.path.join(atlas_output_path, fname)) or os.path.exists(os.path.join(atlas_output_path, gz_fname)):
                 success = True
                 logger.info('ATLAS RUN #{} COMPLETED SUCCESSFULLY'.format(atlas_run_count))
            else:
                 logger.error('ATLAS RUN #{} FAILED: Expected output file {} not found.'.format(atlas_run_count, fname))
        except Exception as e:
            logger.error('ATLAS RUN #{} FAILED with exception: {}'.format(atlas_run_count, e))

        if not success:
            atlas_run_count += 1

    if not success:
        logger.critical(f"ATLAS failed after {atlas_run_count-1} retries. Expected output file {fname} not found.")
        sys.exit(1)


def generate_activity_plans(settings, year, state: WorkflowState, client, resume_after=None, warm_start=False):
    """
    Parameters
    ...
    if settings.get('regenerate_seed', True):
        ...
        try:
            ...
            state.record_input_file(activity_demand_model, asim_config_path, description="ActivitySim settings file before seed update")

            asim_pre.update_asim_config(settings, state.full_path, "random_seed", new_seed)

            # Record output (modified config file)
            state.record_output_file(activity_demand_model, asim_config_path, description="ActivitySim settings file after seed update")
    ...
    if activity_demand_model == 'polaris':
        ...
        if not state.asim_compiled:
            ...
            asim_compile_run_index = state.record_model_start(activity_demand_model, year=state.forecast_year, iteration=state.current_inner_iter, description="ActivitySim Compilation Run")
            ...
            state.record_model_completion(asim_compile_run_index, status="completed" if success else "failed")
        ...
        asim_main_run_index = state.record_model_start(activity_demand_model, year=state.forecast_year, iteration=state.current_inner_iter, description="ActivitySim Main Run")
        ...
        state.record_model_completion(asim_main_run_index, status="completed" if success else "failed")
        if not success:
        ...
        asim_output_dir = os.path.join(state.full_path, settings['asim_local_output_folder'])
        ...
        for table_name in output_tables:
             if file_format == "csv":
                  ...
                  file_path = os.path.join(asim_output_dir, settings['asim_local_output_folder'], file_name) # This path seems wrong, should be asim_output_dir directly?
                  # Corrected path assumption:
                  file_path = os.path.join(asim_output_dir, file_name)
                  if os.path.exists(file_path):
                       state.record_output_file(activity_demand_model, file_path, year=state.forecast_year, description=f"ActivitySim final output table: {table_name} (CSV)")
             elif file_format == "parquet":
                  # Parquet outputs are typically in a 'final_pipeline' subdir
                  file_path = os.path.join(asim_output_dir, "final_pipeline", table_name, "final.parquet")
                  if os.path.exists(file_path):
                       state.record_output_file(activity_demand_model, file_path, year=state.forecast_year, description=f"ActivitySim final output table: {table_name} (Parquet)")
        ...
        if (settings.get('land_use_model') is not None) and (not warm_start):
            ...
            usim_input_store_path_prev = os.path.join(state.full_path, settings['usim_local_mutable_data_folder'], usim_post.get_usim_datastore_fname(settings, io='input'))
            state.record_input_file(land_use_model, usim_input_store_path_prev, description="UrbanSim input data from previous iteration")
            usim_output_store_path_prev = os.path.join(state.full_path, settings['usim_local_mutable_data_folder'], usim_post.get_usim_datastore_fname(settings, io='output', year=state.forecast_year))
            state.record_input_file(land_use_model, usim_output_store_path_prev, description="UrbanSim output data from current year")
            ...
            usim_input_store_path_next = os.path.join(state.full_path, settings['usim_local_mutable_data_folder'], usim_post.get_usim_datastore_fname(settings, io='input'))
            state.record_output_file(land_use_model, usim_input_store_path_next, year=state.forecast_year + settings.get('land_use_freq', 1), description="UrbanSim input data for next iteration")
...
def run_traffic_assignment(
        settings, state: WorkflowState, client, iteration_number=0): # Use iteration_number for BEAM
    """
    ...
    if travel_model == 'polaris':
        ...
        expected_config_path_in_run_dir = os.path.join(abs_beam_input, region, beam_config)
        if os.path.exists(expected_config_path_in_run_dir):
            logger.info(f"BEAM config file exists at host path: {expected_config_path_in_run_dir}")
            # Record the config file as an input to the BEAM run itself
            state.record_input_file(travel_model, expected_config_path_in_run_dir, description="BEAM configuration file used for run")
        else:
            logger.warning(f"BEAM config file NOT FOUND at expected host path: {expected_config_path_in_run_dir}")
            # Decide how to handle this - maybe exit? For now, just warn.


        # Record BEAM run start
        beam_run_index = state.record_model_start(travel_model, year=state.forecast_year, iteration=iteration_number)
        ...
        state.record_model_completion(beam_run_index, status="completed" if success else "failed")
        if not success:
        ...
        if os.path.exists(path_to_mutable_od_skims):
             state.record_output_file(travel_model, path_to_mutable_od_skims, year=state.forecast_year, description="BEAM raw OD skims output")
        if os.path.exists(path_to_origin_skims):
             state.record_output_file(travel_model, path_to_origin_skims, year=state.forecast_year, description="BEAM raw origin skims output")
        ...
        if skimFormat == "csv.gz":
            ...
            if current_od_skims == previous_od_skims and iteration_number > 0: # Check for failure in non-first iter
                logger.error(
            ...
            asim_skims_path = os.path.join(asim_mutable_data_dir, settings['skims_fname'])
            if os.path.exists(asim_skims_path):
                 state.record_output_file(travel_model, asim_skims_path, year=state.forecast_year, description="ActivitySim skims input file updated by BEAM postprocessing")
            ...
            if asim_enabled:
                asim_data_dir = os.path.join(state.full_path, settings['asim_local_output_folder'], "cache") if settings["file_format"] == "parquet" else os.path.join(state.full_path, settings['asim_local_mutable_data_folder'])
                asim_skims_path = os.path.join(asim_data_dir, 'skims.zarr') if settings["file_format"] == "parquet" else os.path.join(asim_data_dir, 'skims.omx')

                # Record inputs to BEAM postprocessing (ASim skims and BEAM raw skims)
                if os.path.exists(asim_skims_path):
                     state.record_input_file(travel_model, asim_skims_path, description="ActivitySim skims input file before BEAM update")
                ...
                if os.path.exists(asim_skims_path):
                     state.record_output_file(travel_model, asim_skims_path, year=state.forecast_year, description=f"ActivitySim skims input file updated by BEAM postprocessing ({skimFormat})")
                ...
                current_od_skims = beam_post.find_produced_od_skims(beam_local_output_folder, skimFormat)
                if current_od_skims == previous_od_skims and iteration_number > 0: # Check for failure in non-first iter
                    logger.error(
                        "BEAM hasn't produced the new skims at {0} for some reason. "
        ...
        logger.info(f"Renaming BEAM output directory for year {year}, iteration {iteration_number}")
        beam_post.rename_beam_output_directory(abs_beam_output, settings, year, iteration_number)
...
def postprocess_all(settings, state: WorkflowState):
    ...
    postprocess_run_index = state.record_model_start("postprocessing", year=state.current_year)
    ...
    yearsAndIters = [(os.path.basename(loc).split('-year')[-1].split('-iter')[0], os.path.basename(loc).split('-iter')[-1]) for loc in outputDirs]

    # Process event files for the latest iteration of each year found
    processed_years = set()
    for year_str, iter_str in sorted(yearsAndIters, key=lambda x: (int(x[0]), int(x[1]))): # Process in order
        year = int(year_str)
        iter = int(iter_str)
        if year not in processed_years:
            ...
            try:
                ...
                beam_iter_output_dir = os.path.join(run_beam_output_base, region, f"year{year}-iter{iter}")
                beam_event_file = os.path.join(beam_iter_output_dir, "events.csv.gz")
                if os.path.exists(beam_event_file):
                     state.record_input_file("postprocessing", beam_event_file, description=f"BEAM events file (Year {year}, Iter {iter})")
                else:
                     logger.warning(f"BEAM event file not found for Year {year}, Iter {iter} at {beam_event_file}")


                process_event_file(settings, year, iter, base_output_dir=run_beam_output_base)

                # Record outputs from event file processing (processed CSVs)
                processed_output_dir = os.path.join(run_beam_output_base, region, f"year{year}-iter{iter}", "processed")
                if os.path.exists(processed_output_dir):
                     for file in os.listdir(processed_output_dir):
                          if file.endswith('.csv'):
                               state.record_output_file("postprocessing", os.path.join(processed_output_dir, file), year=year, description=f"Processed BEAM output file: {file} (Year {year}, Iter {iter})")

                processed_years.add(year) # Mark year as processed (only latest iter)
            except Exception as e:
                logger.error(f"Error processing event file for year {year}, iteration {iter}: {e}")
    ...
    state.record_model_completion(postprocess_run_index, status="completed") # Assuming completed even if some steps failed
...
def run_container(client, settings: dict, image: str, volumes: dict, command: str,
    ...
    if client: # Docker client is available
        ...
        try:
            ...
            logger.info(f"Docker container {image} finished with exit code: {exit_code}")
            return exit_code == 0
        ...
        proc = ["singularity", "run", "--cleanenv", "--writable-tmpfs"] \
               ...
               + command.split() # Split command string into list
        ...
        if settings.get('use_stubs'):
            ...
            stub_cmd = [
                'python',
                os.path.join(os.path.dirname(__file__), 'tests', 'stubs', 'run_stub.py'), # Use absolute path for stub
                '--model_name', model_name,
                '--cwd', os.getcwd(), # Pass current working directory of run.py
                '--config_name', command # Pass the original command string
            ]
            logger.info(f"Using stub for {model_name} ({image}). Running stub command: {' '.join(stub_cmd)}")
            try:
                result = subprocess.run(stub_cmd, check=True, capture_output=True, text=True)
                print("Stub stdout:\n", result.stdout)
                print("Stub stderr:\n", result.stderr)
                logger.info(f"Stub for {model_name} finished successfully.")
                return True # Stub success is check=True
            except subprocess.CalledProcessError as e:
                logger.error(f"Stub for {model_name} failed with exit code {e.returncode}.")
                print("Stub stdout:\n", e.stdout)
                print("Stub stderr:\n", e.stderr)
                return False
            except FileNotFoundError:
            ...
            try:
                ...
                result = subprocess.run(proc, check=False) # Don't raise exception on non-zero exit code
                logger.info(f"Singularity container {image} finished with exit code: {result.returncode}")
                return result.returncode == 0
            except FileNotFoundError:
                logger.error(f"Singularity command not found. Is Singularity installed and in your PATH?")
                return False
            except Exception as e:
                logger.error(f"Unexpected error running singularity container {image}: {e}")
                return False
...
def get_model_and_image(settings: dict, model_type: str):
    ...
    if not image_name:
        raise ValueError(f"No {manager} image specified for model '{model_name}' (model type: {model_type}). Check settings['{manager}_images'].")
...
if __name__ == '__main__':
    ...
    logger.info("Using config file {}".format(settings.get('settings_file', 'N/A'))) # Use .get for safety
    ...
    warm_start_activities_enabled = settings.get('warm_start_activities', False) # Use .get with default
    ...
    vehicle_ownership_model_enabled = settings.get('vehicle_ownership_model_enabled', False)  # Atlas, Use .get with default
    activity_demand_enabled = settings.get('activity_demand_enabled', False) # Use .get with default
    traffic_assignment_enabled = settings.get('traffic_assignment_enabled', False) # Use .get with default
    replanning_enabled = settings.get('replanning_enabled', False) # Use .get with default
    container_manager = settings.get('container_manager') # Use .get

    state = WorkflowState.from_settings(settings)
    working_dir = state.full_path # This is the run-specific output directory if specified, else cwd
    ...
    if traffic_assignment_enabled:
        try:
            ...
            beam_config_path_in_run_dir = os.path.join(state.full_path, settings['beam_local_mutable_data_folder'], settings['region'], settings['beam_config'])
            if os.path.exists(beam_config_path_in_run_dir):
                 ...
                 state.record_output_file(settings.get('travel_model', 'beam'), beam_config_path_in_run_dir, description="BEAM config file updated with beam_sample")
            else:
                 logger.warning(f"BEAM config file not found at {beam_config_path_in_run_dir}. Cannot update beam_sample.")
    ...
    for year in state:
        ...
        logger.info(f"Enabled stages: {[s.name for s in state.enabled_individual_stages]}")
        ...
        if state.should_run(WorkflowState.Stage.land_use):
            ...
            if not land_use_enabled:
                ...
                usim_data_path_check = os.path.join(state.full_path, settings['usim_local_mutable_data_folder'],
        ...
        if state.should_run(WorkflowState.Stage.vehicle_ownership_model):
            if not vehicle_ownership_model_enabled:
            ...
            elif state.forecast_year > 2017: # Check if forecast year is valid for Atlas
                # run_atlas_auto handles its own provenance tracking internally
                ...
                logger.info(f"Skipping atlas in year {state.year} because forecast year {state.forecast_year} is not > 2017")
        ...
        while state.should_run(WorkflowState.Stage.supply_demand_loop, state.current_inner_iter):
             ...
             for substage in substage_sequence:
                 if state.should_run(WorkflowState.Stage.supply_demand_loop, iteration, substage):
                     if substage == WorkflowState.Stage.activity_demand:
                     ...
                     elif substage == WorkflowState.Stage.activity_demand_directly_from_land_use:
                         land_use_model = settings.get('land_use_model', False)
                         if not settings.get('land_use_enabled', False) or not land_use_model:
                             ...
                             usim_output_store_name = usim_post.get_usim_datastore_fname(settings, io='output', year=state.forecast_year)
                             usim_output_store_path = os.path.join(state.full_path, settings['usim_local_mutable_data_folder'], usim_output_store_name)
                             state.record_input_file(land_use_model, usim_output_store_path, description="UrbanSim output for direct activity generation")
                             ...
                             usim_input_store_path_next = os.path.join(state.full_path, settings['usim_local_mutable_data_folder'], usim_post.get_usim_datastore_fname(settings, io='input'))
                             state.record_output_file(land_use_model, usim_input_store_path_next, year=state.forecast_year + settings.get('land_use_freq', 1), description="UrbanSim input data for next iteration (from direct activity)")
                         ...
                         if settings['discard_plans_every_year']:
                             ...
                             beam_config_path_in_run_dir = os.path.join(state.full_path, settings['beam_local_mutable_data_folder'], settings['region'], settings['beam_config'])
                             if os.path.exists(beam_config_path_in_run_dir):
                                  beam_pre.update_beam_config(settings, state.full_path, 'max_plans_memory', 0)
                                  state.record_output_file(settings.get('travel_model', 'beam'), beam_config_path_in_run_dir, description=f"BEAM config file updated with max_plans_memory=0 (Iter {iteration+1})")
                             else:
                                  logger.warning(f"BEAM config file not found at {beam_config_path_in_run_dir}. Cannot update max_plans_memory.")

                         else:
                             # Update BEAM config in the mutable run directory
                             beam_config_path_in_run_dir = os.path.join(state.full_path, settings['beam_local_mutable_data_folder'], settings['region'], settings['beam_config'])
                             if os.path.exists(beam_config_path_in_run_dir):
                                  beam_pre.update_beam_config(settings, state.full_path, 'max_plans_memory') # Update with default/settings value
                                  state.record_output_file(settings.get('travel_model', 'beam'), beam_config_path_in_run_dir, description=f"BEAM config file updated with max_plans_memory (Iter {iteration+1})")
                             else:
                                  logger.warning(f"BEAM config file not found at {beam_config_path_in_run_dir}. Cannot update max_plans_memory.")
                         ...
                         if vehicle_ownership_model_enabled:
                             ...
                             atlas_vehicles_file = os.path.join(atlas_output_path, f'vehicles_{state.forecast_year}.csv.gz') # Assuming gzipped output
                             if os.path.exists(atlas_vehicles_file):
                                  state.record_input_file(settings.get('travel_model', 'beam'), atlas_vehicles_file, description=f"Atlas vehicles output for BEAM input (Iter {iteration+1})")
                             else:
                                  logger.warning(f"Atlas vehicles file not found at {atlas_vehicles_file}. Cannot copy to BEAM.")
                             ...
                             if os.path.exists(beam_vehicles_path):
                                  state.record_output_file(settings.get('travel_model', 'beam'), beam_vehicles_path, year=state.forecast_year, description=f"BEAM vehicles input file updated from Atlas (Iter {iteration+1})")
                     ...
                     state.complete_step(WorkflowState.Stage.supply_demand_loop, iteration, substage)

...
class WorkflowState:
    ...
    def __init__(self, start_year: int, end_year: int, travel_model_freq: int, land_use_enabled: bool,
        ...
        if self.output_path and self.run_id:
             ...
             if self.folder_name:
                 ...
                 tracker_output_path = self.output_path # Should not happen if folder_name is None with output_path
             os.makedirs(tracker_output_path, exist_ok=True)
             self.provenance_tracker = ProvenanceTracker(self.run_id, self.output_path, self.folder_name)
    ...
    def _create_output_dir(self, settings: dict):
        ...
        for model_key in ['travel_model', 'activity_demand_model', 'vehicle_ownership_model', 'land_use_model']:
            ...
            if model_name:
                ...
                if (model_name == "urbansim") or ((model_name == "activitysim") and have_not_copied_usim_data):
                        ...
                        for file in os.listdir(input_dir):
                            ...
                            if os.path.isfile(input_path):
                                self.record_input_file(
                                    model_name, input_path, description="Base UrbanSim input data"
                                )
                    ...
                    if os.path.exists(beam_input_dir):
                        for root, dirs, files in os.walk(beam_input_dir):
                            for file in files:
                                ...
                                if os.path.isfile(input_path):
                                     self.record_input_file(
                                        model_name, input_path, description="BEAM configuration/input file"
                                    )
                    ...
                    if atlas_input_dir and os.path.exists(atlas_input_dir):
                        for root, dirs, files in os.walk(atlas_input_dir):
                            for file in files:
                                ...
                                if os.path.isfile(input_path):
                                    self.record_input_file(
                                        model_name, input_path, description="Atlas input file"
                                    )
                    ...
                    if asim_config_dir and os.path.exists(asim_config_dir):
                        for root, dirs, files in os.walk(asim_config_dir):
                            for file in files:
                                ...
                                if os.path.isfile(input_path):
                                    self.record_input_file(
                                        model_name, input_path, description="ActivitySim configuration file"
                                    )
    ...
    def record_model_start(self, model: str, year: int = None, iteration: int = None) -> int:
    ...
    def record_input_file(self, model: str, file_path: str, source_run_id: str = None, description: str = None):
    ...
    def from_settings(cls, settings):
        ...
        if output_directory is not None:
            ...
            if out.output_path is None or out.folder_name is None:
                 ...
                 logger.info(f"Resuming run {out.run_id} in existing output directory: {out.full_path}")
                 ...
                 if out.provenance_tracker and out.provenance_tracker.run_info.get("settings_hash") is None:
        ...
        if year:
            ...
            next_year_candidate = out.current_year + (out.initial_step if out.current_year == 2010 else out.travel_model_freq)
            out.forecast_year = min(next_year_candidate, out.end_year) if land_use_enabled else out.start_year
        ...
        if target_sub_stage is not None:
            ...
            if self.current_sub_stage is None:
                ...
                try:
                    ...
                    logger.error(f"Target substage {target_sub_stage.name} not found in sequence")
                    return False
    ...
    def __next__(self):
        ...
        if self.current_year is not None and self.current_year > self.end_year and self.current_major_stage is None:
        ...
        if self.current_year is None and self.current_major_stage is None:
             logger.warning("Workflow state indicates no current year and no active stage. Stopping.")
             raise StopIteration

        # If we are past the end year but still have an active stage, something is wrong
        if self.current_year is not None and self.current_year > self.end_year and self.current_major_stage is not None:
             logger.error(f"Workflow is past end year ({self.end_year}) but still in stage {self.current_major_stage.name}. Stopping.")
             raise StopIteration
    ...
    def complete_step(self, completed_major: Stage, completed_inner_iter: int = 0, completed_sub: Optional[Stage] = None):
        ...
        if completed_sub is not None:
            if self.current_major_stage != self.Stage.supply_demand_loop:
                 logger.error(f"Completed substage {completed_sub.name} but not in supply-demand loop. State inconsistency.")
            ...
            try:
                ...
                logger.error(f"Completed substage {completed_sub.name} not found in sequence. State inconsistency.")
                # Attempt to recover by advancing major stage
                self._advance_to_next_major_stage()
        ...
        # Handle major stage completion
             ...
             if self.current_major_stage != completed_major:
                  logger.error(f"Completed major stage {completed_major.name} but current major stage is {self.current_major_stage.name}. State inconsistency.")
    ...
    def _advance_to_next_major_stage(self):
        ...
        try:
            ...
            logger.error(f"Current major stage {self.current_major_stage.name} not found in order. State inconsistency.")
    ...
    def _advance_to_next_year(self):
        ...
        if self.current_year <= self.end_year:
            ...
            next_year_candidate = self.current_year + (self.initial_step if self.current_year == 2010 else self.travel_model_freq)
            self.forecast_year = min(next_year_candidate, self.end_year) if self._settings.get('land_use_enabled') else self.start_year

        else:
            logger.info(f"Workflow complete at end year {self.end_year}")
            self.current_year = self.end_year + 1 # Set year just past end_year to signal completion
    ...
    def get_sub_stages_from(self, start_sub_stage: Optional[Stage]):
        ...
        try:
            ...
            logger.error("Start sub-stage %s not found in sequence. Restarting from beginning.", start_sub_stage.name if start_sub_stage else None)
            return self.loop_substages

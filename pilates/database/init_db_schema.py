...
def main():
    parser = argparse.ArgumentParser(description="Initialize a DuckDB database with the PILATES schema.")
    parser.add_argument("database_path", help="Path to the DuckDB database file to initialize.")
    ...
    try:
        ...
        if db_manager.initialize_database():
            logging.info(f"Successfully initialized database schema at {args.database_path}")
        else:
            logging.error(f"Failed to initialize database schema at {args.database_path}")

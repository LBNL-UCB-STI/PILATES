...
COMMENT ON TABLE file_records IS 'Individual datasets with complete provenance lineage, linking files across model stages';
COMMENT ON COLUMN file_records.unique_id IS 'File content hash (SHA256) serving as unique identifier';
COMMENT ON COLUMN file_records.record_type IS 'Type of record: file, h5_container, or h5_table';
COMMENT ON COLUMN file_records.run_id IS 'Foreign key to runs table indicating which run produced/used this file';
...
COMMENT ON COLUMN file_records.year IS 'Simulation year this file corresponds to (NULL if not year-specific)';
...
COMMENT ON COLUMN file_records.exists IS 'Boolean indicating if file still exists on disk';

-- H5 file-table relationships

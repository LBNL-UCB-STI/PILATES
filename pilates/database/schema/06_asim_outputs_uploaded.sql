...
CREATE TABLE IF NOT EXISTS uploaded_households_asim_out (
...
COMMENT ON TABLE uploaded_households_asim_out IS 'ActivitySim output file: households - Socio-economic and demographic characteristics of households, including income, size, auto ownership, and worker counts.';

CREATE INDEX IF NOT EXISTS idx_uploaded_households_asim_out_run_id ON uploaded_households_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_households_asim_out_year_iter_sub_iter ON uploaded_households_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_households_asim_out_household_id ON uploaded_households_asim_out(household_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_households_asim_out_home_zone_id ON uploaded_households_asim_out(home_zone_id);
...
CREATE TABLE IF NOT EXISTS uploaded_persons_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
    ...
    FOREIGN KEY (run_id) REFERENCES runs(run_id),
    FOREIGN KEY (file_record_id) REFERENCES file_records(unique_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, household_id) REFERENCES uploaded_households_asim_out(run_id, year, iteration, sub_iteration, household_id)
);

COMMENT ON TABLE uploaded_persons_asim_out IS 'ActivitySim output file: persons - Detailed information about individuals, including demographics (age, sex), employment and student status, home and workplace locations, and various travel-related attributes.';

CREATE INDEX IF NOT EXISTS idx_uploaded_persons_asim_out_run_id ON uploaded_persons_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_persons_asim_out_year_iter_sub_iter ON uploaded_persons_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_persons_asim_out_person_id ON uploaded_persons_asim_out(person_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_persons_asim_out_household_id ON uploaded_persons_asim_out(household_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_persons_asim_out_home_zone_id ON uploaded_persons_asim_out(home_zone_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_persons_asim_out_school_zone_id ON uploaded_persons_asim_out(school_zone_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_persons_asim_out_workplace_zone_id ON uploaded_persons_asim_out(workplace_zone_id);
...
CREATE TABLE IF NOT EXISTS uploaded_tours_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
    ...
    FOREIGN KEY (run_id) REFERENCES runs(run_id),
    FOREIGN KEY (file_record_id) REFERENCES file_records(unique_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, person_id) REFERENCES uploaded_persons_asim_out(run_id, year, iteration, sub_iteration, person_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, household_id) REFERENCES uploaded_households_asim_out(run_id, year, iteration, sub_iteration, household_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, parent_tour_id) REFERENCES uploaded_tours_asim_out (run_id, year, iteration, sub_iteration, tour_id)
);

COMMENT ON TABLE uploaded_tours_asim_out IS 'ActivitySim output file: tours - Detailed information about tours, including purpose, participants, and travel characteristics, linking to persons and households.';

CREATE INDEX IF NOT EXISTS idx_uploaded_tours_asim_out_run_id ON uploaded_tours_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_tours_asim_out_year_iter_sub_iter ON uploaded_tours_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_tours_asim_out_tour_id ON uploaded_tours_asim_out(tour_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_tours_asim_out_person_id ON uploaded_tours_asim_out(person_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_tours_asim_out_household_id ON uploaded_tours_asim_out(household_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_tours_asim_out_parent_tour_id ON uploaded_tours_asim_out(parent_tour_id);
...
CREATE TABLE IF NOT EXISTS uploaded_accessibility_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
...
COMMENT ON TABLE uploaded_accessibility_asim_out IS 'ActivitySim output file: accessibility - Accessibility measures by zone, including auto, transit, and non-motorized modes for retail and total destinations during peak and off-peak hours.';

CREATE INDEX IF NOT EXISTS idx_uploaded_accessibility_asim_out_run_id ON uploaded_accessibility_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_accessibility_asim_out_year_iter_sub_iter ON uploaded_accessibility_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_accessibility_asim_out_zone_id ON uploaded_accessibility_asim_out(zone_id);
...
CREATE TABLE IF NOT EXISTS uploaded_trips_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
    ...
    FOREIGN KEY (run_id, year, iteration, sub_iteration, person_id) REFERENCES uploaded_persons_asim_out(run_id, year, iteration, sub_iteration, person_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, household_id) REFERENCES uploaded_households_asim_out(run_id, year, iteration, sub_iteration, household_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, tour_id) REFERENCES uploaded_tours_asim_out(run_id, year, iteration, sub_iteration, tour_id)
);

COMMENT ON TABLE uploaded_trips_asim_out IS 'ActivitySim output file: trips - Detailed information about individual trips, including origin, destination, mode, and purpose, linking to persons, households, and tours.';

CREATE INDEX IF NOT EXISTS idx_uploaded_trips_asim_out_run_id ON uploaded_trips_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_trips_asim_out_year_iter_sub_iter ON uploaded_trips_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_trips_asim_out_trip_id ON uploaded_trips_asim_out(trip_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_trips_asim_out_person_id ON uploaded_trips_asim_out(person_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_trips_asim_out_household_id ON uploaded_trips_asim_out(household_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_trips_asim_out_tour_id ON uploaded_trips_asim_out(tour_id);
...
CREATE TABLE IF NOT EXISTS uploaded_beam_plans_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
    ...
    FOREIGN KEY (run_id, year, iteration, sub_iteration, person_id) REFERENCES uploaded_persons_asim_out(run_id, year, iteration, sub_iteration, person_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, tour_id) REFERENCES uploaded_tours_asim_out(run_id, year, iteration, sub_iteration, tour_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, trip_id) REFERENCES uploaded_trips_asim_out(run_id, year, iteration, sub_iteration, trip_id)
);

COMMENT ON TABLE uploaded_beam_plans_asim_out IS 'ActivitySim output file: beam_plans - Detailed trip plans generated by ActivitySim, including information about tours, trips, modes, activities, and costs.';

CREATE INDEX IF NOT EXISTS idx_uploaded_beam_plans_asim_out_run_id ON uploaded_beam_plans_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_beam_plans_asim_out_year_iter_sub_iter ON uploaded_beam_plans_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_beam_plans_asim_out_trip_id ON uploaded_beam_plans_asim_out(trip_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_beam_plans_asim_out_person_id ON uploaded_beam_plans_asim_out(person_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_beam_plans_asim_out_tour_id ON uploaded_beam_plans_asim_out(tour_id);
...
CREATE TABLE IF NOT EXISTS uploaded_disaggregate_accessibility_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
...
COMMENT ON TABLE uploaded_disaggregate_accessibility_asim_out IS 'ActivitySim output file: disaggregate_accessibility - Accessibility measures at the individual person level, including workplace, other discretionary, and shopping activities.';

CREATE INDEX IF NOT EXISTS idx_uploaded_disaggregate_accessibility_asim_out_run_id ON uploaded_disaggregate_accessibility_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_disaggregate_accessibility_asim_out_year_iter_sub_iter ON uploaded_disaggregate_accessibility_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_disaggregate_accessibility_asim_out_person_id ON uploaded_disaggregate_accessibility_asim_out(person_id);
...
CREATE TABLE IF NOT EXISTS uploaded_joint_tour_participants_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
...
COMMENT ON TABLE uploaded_joint_tour_participants_asim_out IS 'ActivitySim output file: joint_tour_participants - Records the participation of individuals in joint tours, linking to specific tours, households, and persons.';

CREATE INDEX IF NOT EXISTS idx_uploaded_joint_tour_participants_asim_out_run_id ON uploaded_joint_tour_participants_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_joint_tour_participants_asim_out_year_iter_sub_iter ON uploaded_joint_tour_participants_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_joint_tour_participants_asim_out_tour_id ON uploaded_joint_tour_participants_asim_out(tour_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_joint_tour_participants_asim_out_household_id ON uploaded_joint_tour_participants_asim_out(household_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_joint_tour_participants_asim_out_person_id ON uploaded_joint_tour_participants_asim_out(person_id);
...
CREATE TABLE IF NOT EXISTS uploaded_land_use_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
...
COMMENT ON TABLE uploaded_land_use_asim_out IS 'ActivitySim output file: land_use - Land use characteristics by traffic analysis zone (TAZ), including population, households, employment, and various demographic and economic indicators.';

CREATE INDEX IF NOT EXISTS idx_uploaded_land_use_asim_out_run_id ON uploaded_land_use_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_land_use_asim_out_year_iter_sub_iter ON uploaded_land_use_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_land_use_asim_out_zone_id ON uploaded_land_use_asim_out(zone_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_land_use_asim_out_county_id ON uploaded_land_use_asim_out(county_id);
...
CREATE TABLE IF NOT EXISTS uploaded_non_mandatory_tour_destination_accessibility_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
    ...
    FOREIGN KEY (run_id) REFERENCES runs(run_id),
    FOREIGN KEY (file_record_id) REFERENCES file_records(unique_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, proto_person_id) REFERENCES uploaded_persons_asim_out(run_id, year, iteration, sub_iteration, person_id),
    FOREIGN KEY (run_id, year, iteration, sub_iteration, proto_household_id) REFERENCES uploaded_households_asim_out(run_id, year, iteration, sub_iteration, household_id)
);

COMMENT ON TABLE uploaded_non_mandatory_tour_destination_accessibility_asim_out IS 'ActivitySim output file: non_mandatory_tour_destination_accessibility - Provides accessibility measures for non-mandatory tour destinations, enriched with detailed person, household, and land-use attributes.';

CREATE INDEX IF NOT EXISTS idx_uploaded_non_mandatory_tour_destination_accessibility_asim_out_run_id ON uploaded_non_mandatory_tour_destination_accessibility_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_non_mandatory_tour_destination_accessibility_asim_out_year_iter_sub_iter ON uploaded_non_mandatory_tour_destination_accessibility_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_non_mandatory_tour_destination_accessibility_asim_out_proto_person_id ON uploaded_non_mandatory_tour_destination_accessibility_asim_out(proto_person_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_non_mandatory_tour_destination_accessibility_asim_out_proto_household_id ON uploaded_non_mandatory_tour_destination_accessibility_asim_out(proto_household_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_non_mandatory_tour_destination_accessibility_asim_out_proto_tour_id ON uploaded_non_mandatory_tour_destination_accessibility_asim_out(proto_tour_id);
...
CREATE TABLE IF NOT EXISTS uploaded_person_windows_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
...
COMMENT ON TABLE uploaded_person_windows_asim_out IS 'ActivitySim output file: person_windows - Records the activity status or availability of each person during different hours of the day.';

CREATE INDEX IF NOT EXISTS idx_uploaded_person_windows_asim_out_run_id ON uploaded_person_windows_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_person_windows_asim_out_year_iter_sub_iter ON uploaded_person_windows_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_person_windows_asim_out_person_id ON uploaded_person_windows_asim_out(person_id);
...
CREATE TABLE IF NOT EXISTS uploaded_school_destination_size_asim_out (
    run_id VARCHAR NOT NULL,
    file_record_id VARCHAR NOT NULL,
    year INTEGER NOT NULL,
    iteration INTEGER NOT NULL,
...
COMMENT ON TABLE uploaded_school_destination_size_asim_out IS 'ActivitySim output file: school_destination_size - Provides measures of school destination attractiveness or size for universities, gradeschools, and highschools within each traffic analysis zone.';

CREATE INDEX IF NOT EXISTS idx_uploaded_school_destination_size_asim_out_run_id ON uploaded_school_destination_size_asim_out(run_id);
CREATE INDEX IF NOT EXISTS idx_uploaded_school_destination_size_asim_out_year_iter_sub_iter ON uploaded_school_destination_size_asim_out(year, iteration, sub_iteration);
CREATE INDEX IF NOT EXISTS idx_uploaded_school_destination_size_asim_out_zone_id ON uploaded_school_destination_size_asim_out(zone_id);

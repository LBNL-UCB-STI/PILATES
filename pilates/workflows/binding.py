...
def _restart_atlas_required_artifacts(
    ...
    **_: Any,
    ...
    # Restart executes vehicle ownership from its stage boundary.  It does not
    # resume inside an ATLAS preprocess/run/postprocess substage, so the
    # per-year CSV seeds must be recreated by ``atlas_preprocess`` rather than
    # demanded by preflight before that step can run.
    return required or None

...
def _semantic_product(path: Path, *, workspace_root: Path) -> dict[str, Any]:
    ...
    if path.is_file():
        return {"relative_path": relative_path, "present": True, "type": "file", "size": path.stat().st_size}
    if not path.is_dir():
    ...
    return {"relative_path": relative_path, "present": True, "type": "directory", "members": members}
...
def run_phase(
    *,
    ...
    if isinstance(cache_source, Mapping):
        ...
        source_run_id = str(value) if value is not None else None
...
def _phase_record(execution: PhaseExecution, workspace_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "cohort": {"year": execution.cohort_year, "iteration": execution.cohort_iteration},
    }
...
def _validate(cold: dict[str, Any], fresh: dict[str, Any]) -> dict[str, Any]:
    ...
    present = all(item.get("present") is True for item in cold["declared_outputs"].values()) and all(item.get("present") is True for item in fresh["declared_outputs"].values())
    products_match = keys_match and cold["declared_outputs"] == fresh["declared_outputs"]
    body_valid = cold["body_executions_after"] == cold["body_executions_before"] + 1 and fresh["body_executions_after"] == cold["body_executions_after"]
    valid = not differences and present and products_match and body_valid and cold["workspace_root"] != fresh["workspace_root"]
    return {
        "valid": valid,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    state.current_inner_iter = _ACCEPTANCE_ITERATION

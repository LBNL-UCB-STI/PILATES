...
def _update_persons_table(persons, households, unassigned_households, blocks, asim_zone_id_col='TAZ'):
    ...
    # Assign zones efficiently using map operation
    household_zones = blocks[asim_zone_id_col].reindex(households['block_id'])
    persons[asim_zone_id_col] = persons['household_id'].map(household_zones).astype(str)

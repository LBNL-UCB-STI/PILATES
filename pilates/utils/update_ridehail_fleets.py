...
def update_fleet_files(trips_df, settings):
    ...
    for f in FLEETS:
        ...
        data = scale_data(data, 1 / (SCALE_FACTOR+.02), OVERLAP['both'], OVERLAP[f],
        ...
        sum_data = data[
            ['Hour', 'prediction_ActiveVeh', 'prediction_DeltaVeh', 'Delta_positive', 'Delta_negative']].groupby(
            'Hour').sum().reset_index().sort_values('Hour')
        sum_data['Adjustment'] = sum_data['prediction_ActiveVeh'].shift(-1).fillna(0) - (
                sum_data['prediction_ActiveVeh'].fillna(0) + sum_data['prediction_DeltaVeh'].fillna(0))
        ...
        data.loc[
            (data.Adjustment > 0) & (
                    data.Delta_positive_total > 0),
            'adj_DeltaVeh'] = data.loc[(data.Adjustment > 0) & (data.Delta_positive_total > 0), 'adj_DeltaVeh'] + \
                              data.loc[(data.Adjustment > 0) & (data.Delta_positive_total > 0), 'Adjustment'] * (
                                      data.loc[(
                                                       data.Adjustment > 0) & (
                                                       data.Delta_positive_total > 0), 'Delta_positive'] /
                                      data.loc[(
                                                       data.Adjustment > 0) & (
                                                       data.Delta_positive_total > 0), 'Delta_positive_total'])
        data.loc[
            (data.Adjustment < 0) & (
                    data.Delta_negative_total < 0),
            'adj_DeltaVeh'] = data.loc[(
                                               data.Adjustment < 0) & (
                                               data.Delta_negative_total < 0), 'adj_DeltaVeh'] + \
                              data.loc[(
                                               data.Adjustment < 0) & (
                                               data.Delta_negative_total < 0), 'Adjustment'] * (
                                      data.loc[(
                                                       data.Adjustment < 0) & (
                                                       data.Delta_negative_total < 0), 'Delta_negative'] /
                                      data.loc[(
                                                       data.Adjustment < 0) & (
                                                       data.Delta_negative_total < 0), 'Delta_negative_total'])
...
def get_data(df, zipShp):
    ...
    num_requests = df[['depart', 'origin', 'destination']].groupby(['depart', 'origin']).count().reset_index().rename(
        columns={'depart': 'Hour', 'origin': 'Taz', 'destination': 'Trip requests'})
    num_ends = df[['depart', 'origin', 'destination']].groupby(['depart', 'destination']).count().reset_index().rename(
        columns={'depart': 'Hour', 'destination': 'Taz', 'origin': 'Trip ends'})
    ...
    Bounds = zipShp[['City', 'minx', 'miny']].groupby(['City']).min().reset_index().merge(
        zipShp[['City', 'maxx', 'maxy']].groupby(['City']).max().reset_index(), how='left', left_on=['City'],
        right_on=['City'])
...
def load_geoData(zip_filename, tazFilename):
    ...
    zipShp = taz_join[['Zip', 'County', 'maxx', 'maxy', 'City', 'Taz']].groupby(
        ['Zip', 'County', 'City', 'Taz']).max().reset_index()
    zipShp = zipShp.merge(taz_join[['Zip', 'County', 'minx', 'miny', 'City', 'Taz']].groupby(
        ['Zip', 'County', 'City', 'Taz']).min().reset_index(),
                          how='left', left_on=['Zip', 'County', 'City', 'Taz'],
                          right_on=['Zip', 'County', 'City', 'Taz'])
    return zipShp

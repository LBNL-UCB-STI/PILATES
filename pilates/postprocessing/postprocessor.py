...
def copy_outputs_to_mep(settings, year, iter, workspace: Workspace):
    ...
    def copy_asim_files_to_mep():
        ...
        copy_with_compression_asim_file_to_mep(
            "final_households.csv", "households.csv.gz"
        )
    ...
    def copy_beam_files_to_mep(workspace: Workspace):
        ...
        linkstats_path = os.path.join(
            beam_iter_output_dir, "ITERS", "it.0", "0.linkstats.csv.gz"
        )
        try:
            shutil.copy(
        ...
        ridehailSkims = os.path.join(
            beam_iter_output_dir, "ITERS", "it.0", "0.skimsRidehail.csv.gz"
        )
        try:
            shutil.copy(
                ridehailSkims, os.path.join(mep_output_data_dir, "ridehailSkims.csv.gz")
            )
        except Exception as e:
            logger.error("Missing expected beam output file {0}".format(ridehailSkims), exc_info=e)
        odSkims = os.path.join(
            beam_iter_output_dir, "ITERS", "it.0", "0.skimsTAZ.csv.gz"
        )
        try:
            shutil.copy(odSkims, os.path.join(mep_output_data_dir, "odSkims.csv.gz"))
        except Exception as e:
            logger.error("Missing expected beam output file {0}".format(odSkims), exc_info=e)
        ...
        for file in os.listdir(beam_router_dir):
            if file.endswith(".zip"):
                shutil.copy(
                    os.path.join(beam_router_dir, file),
                    os.path.join(mep_gtfs_dir, file),
                )
        try:
            shutil.copy(
            ...
            shutil.copy(
                os.path.join(beam_iter_output_dir, "totalsByModeAndTAZ.csv"),
                os.path.join(mep_gtfs_dir, "totalsByModeAndTAZ.csv"),
            )
        except Exception as e:
            logger.error(
                "Totals by mode were not generated by the postprocessor, expected at {0}".format(
                    os.path.join(beam_iter_output_dir, "totalsByMode.csv")
                ),
                exc_info=e
            )
...
def _load_events_file(settings, year, replanning_iteration_number, beam_iteration=0):
    ...
    events_dir = os.path.join(
        "ITERS",
        "it.{0}".format(beam_iteration),
        "{0}.events.csv.gz".format(beam_iteration),
    )
...
def _reformat_events_file(events):
    ...
    events = events[
        ~events.person.str.contains("Agent", na=False) & ~initialParkingEvents
    ...
    events["personID"] = np.where(
        events["person"].isin(events["driver"]), events["person"], np.nan
    )
    events["driverID"] = np.where(
        events["driver"].isin(events["person"]), events["driver"], np.nan
    )
    ...
    events["riderID"] = np.where(
        events["riders"].isin(events["person"]), events["riders"], np.nan
    )
    ...
    # these rows
    events = events[
        ~((events.driver.str.contains("Agent", na=False)) & (events.IDMerged.isna()))
    ].reset_index(drop=True)

    events["chargeID"] = events.groupby("vehicle")["IDMerged"].transform(
    ...
    events["modeBEAM_rh"] = np.where(
        events["type"] == "TeleportationEvent",
        events.modeBEAM_rh.fillna(method="ffill"),
        events["modeBEAM_rh"],
    )
    events["modeBEAM_rh_pooled"] = np.where(
        (events["type"] == "PersonCost") & (events["modeBEAM"] == "ride_hail_pooled"),
        "ride_hail_pooled",
        np.nan,
    )
    events["modeBEAM_rh_ride_hail_transit"] = np.where(
        (events["type"] == "PersonCost") & (events["modeBEAM"] == "ride_hail_transit"),
        "ride_hail_transit",
        np.nan,
    )
    events["modeBEAM_rh_pooled"] = events["modeBEAM_rh_pooled"].shift(+1)
    events["modeBEAM_rh_ride_hail_transit"] = events[
        "modeBEAM_rh_ride_hail_transit"
    ].shift(+1)
    events["modeBEAM_rh"] = np.where(
...
def _expand_events_file(events):
    ...
    events["actStartTime"] = np.where(
        events["type"] == "actstart", events["time"], np.nan
    )
    events["duration_travelling"] = np.where(
        (events["type"] == "PathTraversal") | (events["type"] == "TeleportationEvent"),
        events["arrivalTime"] - events["departureTime"],
        np.nan,
    )
    events["distance_travelling"] = np.where(
        (events["type"] == "PathTraversal")
        | (
            (events["type"] == "ModeChoice")
            & (
    ...
    events["duration_in_ridehail"] = np.where(
        ...
        | (events["modeBEAM_rh"] == "ride_hail_transit"),
    ...
    events["replanningTime"] = np.where(
        events["type"] == "Replanning", events["time"], np.nan
    )
    events["replanningTime"] = events["replanningTime"].shift(+1)
    events["tourIndex_fixed"] = np.where(
        (events["type"] == "ModeChoice") & (events["replanningTime"].notna()),
        np.nan,
        events["tourIndex"],
    )
    ...
    events["fuelDiesel"] = np.where(
        (events["type"] == "PathTraversal") & (events["primaryFuelType"] == "Diesel"),
        events["primaryFuel"],
        np.nan,
    )
    ...
    conditions = [
        (events["modeBEAM_rh"] == "ride_hail_pooled"),
        (events["modeBEAM_rh"] == "walk_transit")
        | (events["modeBEAM_rh"] == "drive_transit")
        | (events["modeBEAM_rh"] == "ride_hail_transit")
        | (events["modeBEAM_rh"] == "bus")
        | (events["modeBEAM_rh"] == "subway")
        | (events["modeBEAM_rh"] == "rail")
        | (events["modeBEAM_rh"] == "tram")
    ...
    events["actStartType"] = np.where(
        events["type"] == "actstart", events["actType"], ""
    )
    events["tripIndex"] = events.tripId.fillna(method="ffill")
    events["mode_choice_actual_BEAM"] = events.groupby(["IDMerged", "tripId", "type"])[
        "modeBEAM"
    ].transform("last")
    events["mode_choice_planned_BEAM"] = events.groupby(["IDMerged", "tripId", "type"])[
        "modeBEAM"
    ].transform("first")
    events["mode_choice_actual_BEAM"] = np.where(
        events["type"] != "ModeChoice", np.nan, events["mode_choice_actual_BEAM"]
    )
    events["mode_choice_planned_BEAM"] = np.where(
        events["type"] != "ModeChoice", np.nan, events["mode_choice_planned_BEAM"]
    )
    ...
    events["ride_hail_pooled"] = np.where(
        events["modeBEAM_rh"] == "ride_hail_pooled", 1, 0
    )
    return events


def _add_geometry_id_to_DataFrame(
    df, gdf, xcol, ycol, idColumn="geometry", df_geom="epsg:4326"
):
    ...
    gdf_data = gdf_data.merge(
        joined["blkgrpid"], left_index=True, right_index=True, how="left"
    )
...
def _add_geometry_to_events(settings, events):
    ...
    with warnings.catch_warnings():
        ...
        processed_list = Parallel(n_jobs=cpu_count() - 1)(
            delayed(_add_geometry_id_to_DataFrame)(
                ev, taz, "startX", "startY", "BlockGroupStart"
            )
            for ev in np.array_split(events, cpu_count() - 1)
        )
        processed_list = Parallel(n_jobs=cpu_count() - 1)(
            delayed(_add_geometry_id_to_DataFrame)(
                ev, taz, "endX", "endY", "BlockGroupEnd"
            )
...
def _build_person_trip_events(events):
    ...
    processed_list = Parallel(n_jobs=cpu_count() - 1)(
        delayed(_aggregate_on_trip)(group, name) for name, group in gb
    )
...
def _process_person_trip_events(person_trip_events):
    person_trip_events["duration_door_to_door"] = (
        person_trip_events["actStartTime"] - person_trip_events["actEndTime"]
    )
    person_trip_events["waitTime_no_replanning"] = np.where(
        person_trip_events["replanning_status"] == 0,
        person_trip_events["duration_door_to_door"]
        - person_trip_events["duration_travelling"],
        0,
    )
    person_trip_events["waitTime_replanning"] = np.where(
        person_trip_events["replanning_status"] > 0,
        person_trip_events["duration_door_to_door"]
        - person_trip_events["duration_travelling"],
        0,
    )
    person_trip_events["actPurpose"] = (
        person_trip_events["actEndType"].astype(str)
        + "_to_"
    ...
    person_trip_events["mode_choice_actual_5"] = np.select(
    ...
    person_trip_events["mode_choice_actual_6"] = np.select(
        conditions, choices, default=np.nan
    )
    # Column with four summarized modes
    person_trip_events["mode_choice_actual_4"] = np.where(
        (person_trip_events["mode_choice_actual_5"] == "walk")
        | (person_trip_events["mode_choice_actual_5"] == "bike"),
        "walk/bike",
        person_trip_events["mode_choice_actual_5"],
    )
    return person_trip_events.sort_values(by=["IDMerged", "tripIndex"]).reset_index(
        drop=False
    )
...
def _merge_trips_with_utilities(asim_trips, asim_utilities, beam_trips):
    ...
    logger.info(
        "Finding {0} BEAM trips not in ASIM plans and {1} ASIM trips not in BEAM events".format(
            len(tripIdsBEAM - tripIdsASIM), len(tripIdsASIM - tripIdsBEAM)
        )
    )
    SFActMerged = (
        pd.merge(left=asim_trips, right=asim_utilities, how="left", on=["trip_id"])
    ...
    eventsASim = pd.merge(
        ...
        right=SFActMerged,
        how="left",
        left_on=["IDMerged", "tripIndex"],
        right_on=["person_id", "trip_id"],
    )
    eventsASim.rename(
        columns={"mode_choice_logsum_y": "logsum_tours_mode_AS_tours"}, inplace=True
    )
    eventsASim.rename(columns={"tour_mode": "tour_mode_AS_tours"}, inplace=True)
    eventsASim.rename(
        columns={"mode_choice_logsum_x": "logsum_trip_Potential_INEXUS"}, inplace=True
    )
    eventsASim.rename(columns={"trip_mode": "trip_mode_AS_trips"}, inplace=True)

    # Add a column of income quartiles
    quartiles = eventsASim["income"].quantile([0, 0.25, 0.5, 0.75, 1]).tolist()
    # Add income deciles
    conditions = [
        (eventsASim["income"] >= quartiles[0]) & (eventsASim["income"] < quartiles[1]),
    ...
    # Add income deciles
    conditions = [
        (eventsASim["income"] >= deciles[0]) & (eventsASim["income"] < deciles[1]),
...
def _read_asim_plans(settings, year, iteration):
    ...
    households = (
        pd.read_csv(os.path.join(path, "households.csv.gz"))
        .sort_values(by=["household_id"])
        .reset_index(drop=True)
    )
    persons = (
        pd.read_csv(os.path.join(path, "persons.csv.gz"))
        .sort_values(by=["household_id"])
        .reset_index(drop=True)
    )
    tours = (
        pd.read_csv(os.path.join(path, "final_tours.csv.gz"))
        .sort_values(by=["person_id"])
        .reset_index(drop=True)
    )
    trips = (
        pd.read_csv(os.path.join(path, "final_trips.csv.gz"))
        .sort_values(by=["person_id", "tour_id"])
        .reset_index(drop=True)
    )
    hhpersons = pd.merge(left=persons, right=households, how="left", on="household_id")
    hhperTours = (
        pd.merge(left=tours, right=hhpersons, how="left", on="person_id")
        .sort_values(by=["person_id", "tour_id"])
        .reset_index(drop=True)
    )
    tour_trips = pd.merge(
        left=trips, right=hhperTours, how="left", on=["person_id", "tour_id"]
...
def build_mep_summaries(trips, settings, year, iteration):
    mode_map = {
        ...
        "bike": "BIKE",
        "hov2_teleportation": "DRIVE",
        "car_hov2": "DRIVE",
        "hov3_teleportation": "DRIVE",
        "car_hov3": "DRIVE",
        "walk": "WALK",
        "ride_hail": "TNC",
        "ride_hail_pooled": "TNC",
        "drive_transit": "TRANSIT",
        "bike_transit": "TRANSIT",
    }
    trips["simplified_mode_BEAM"] = trips["mode_choice_actual_BEAM"].map(
        lambda x: mode_map.get(x, "OTHER")
    )
    totalsByMode = (
        trips.loc[
            :,
        ...
        .groupby("simplified_mode_BEAM")
        .agg(sum)
    )
    totalsByMode["cost_per_passenger_mile"] = (
        totalsByMode["cost_BEAM"] / totalsByMode["distance_mode_choice"] * 1609.34
    )
    totalsByMode["joules_per_passenger_mile"] = (
        totalsByMode["fuel_marginal"] / totalsByMode["distance_mode_choice"] * 1609.34
    )
    ...
    totalsByModeAndTAZ = (
        ...
        .groupby(["bgid_start", "simplified_mode_BEAM"])
        .agg(sum)
    )
    totalsByModeAndTAZ["cost_per_passenger_mile"] = (
        totalsByModeAndTAZ["cost_BEAM"]
        / totalsByModeAndTAZ["distance_mode_choice"]
        * 1609.34
    )
    totalsByModeAndTAZ["joules_per_passenger_mile"] = (
        totalsByModeAndTAZ["fuel_marginal"]
        / totalsByModeAndTAZ["distance_mode_choice"]
        * 1609.34
    )
    ...
    totalsByModeAndTAZ.to_csv(
        os.path.join(
            beam_output_dir, region, iteration_output_dir, "totalsByModeAndTAZ.csv"
        )
    )
...
def process_event_file(settings, year, iteration):
    try:
        ...
        try:
            final_output = _merge_trips_with_utilities(
                tour_trips, utils, person_trip_events
            )
        except Exception as e:
        ...
        final_output.to_csv(
            os.path.join(post_output_folder, filename), compression="gzip"
        )
        ...
        logger.error(
            "Did not successfully run the postproccessor, did activitysim fail? \n {0}".format(
                e
            )
        )
...
if __name__ == "__main__":
    ...
    yearsAndIters = [
        (loc.split("-", 3)[-3], loc.split("-", 3)[-1]) for loc in outputDirs
    ]
    yrs = dict()

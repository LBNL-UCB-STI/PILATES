...
def copy_outputs_to_mep(settings, year, iter):
    ...
    def copy_beam_files_to_mep():
        ...
        try:
            shutil.copy(os.path.join(beam_iter_output_dir, "output_network.xml.gz"),
                        os.path.join(mep_output_data_dir, "output_network.xml.gz"))
        except:
            logger.error("Missing expected beam output file {0}".format(
        ...
        rideHailWaitStats = os.path.join(beam_iter_output_dir, "ITERS", "it.0", "0.rideHailWaitingStats.csv")
        try:
            shutil.copy(rideHailWaitStats, os.path.join(mep_output_data_dir, "rideHailWaitingStats.csv"))
        except:
            logger.error("Missing expected beam output file {0}".format(rideHailWaitStats))
        ...
        try:
            ...
            shutil.copy(os.path.join(beam_iter_output_dir, "totalsByModeAndTAZ.csv"),
...
def _process_person_trip_events(person_trip_events):
    ...
    person_trip_events['waitTime_no_replanning'] = np.where(person_trip_events['replanning_status'] == 0,
                                                            ...
                                                            person_trip_events['duration_travelling'], 0)
    ...
    person_trip_events['mode_choice_actual_4'] = np.where((person_trip_events['mode_choice_actual_5'] == 'walk') | (
            person_trip_events['mode_choice_actual_5'] == 'bike'), 'walk/bike',
...
def build_mep_summaries(trips, settings, year, iteration):
    ...
    trips['simplified_mode_BEAM'] = trips['mode_choice_actual_BEAM'].map(lambda x: mode_map.get(x, "OTHER"))
    totalsByMode = trips.loc[:,
                   ['simplified_mode_BEAM', 'cost_BEAM', 'distance_mode_choice', 'fuel_marginal']].groupby(
        'simplified_mode_BEAM').agg(sum)
    ...
    totalsByModeAndTAZ = trips.loc[:,
                         ['bgid_start', 'simplified_mode_BEAM', 'cost_BEAM', 'distance_mode_choice',
                          'fuel_marginal']].groupby(
        ['bgid_start', 'simplified_mode_BEAM']).agg(sum)
    totalsByModeAndTAZ['cost_per_passenger_mile'] = totalsByModeAndTAZ['cost_BEAM'] / totalsByModeAndTAZ[
        'distance_mode_choice'] * 1609.34
    totalsByModeAndTAZ['joules_per_passenger_mile'] = totalsByModeAndTAZ['fuel_marginal'] / totalsByModeAndTAZ[
        'distance_mode_choice'] * 1609.34
    ...
    totalsByModeAndTAZ.to_csv(os.path.join(beam_output_dir, region, iteration_output_dir, "totalsByModeAndTAZ.csv"))
...
def process_event_file(settings, year, iteration):
    try:
        ...
        filename = "{0}_{1}_{2}-{3}_{4}_iter{6}__{5}.csv.gz".format(settings['region'],
                                                            scenario_defs['name'],
                                                            scenario_defs['lever'],

...
class UrbanSimLaunchContext:
    """Resolved model-visible roots for one UrbanSim container invocation."""

    mutable_data_dir: Path
    output_datastore: Path


class UrbansimRunner(
    GenericRunner[UrbanSimPreprocessOutputs, UrbanSimRunOutputs, UrbanSimLaunchContext]
):
    """
    ...
    def _run(
        ...
        Args:
            inputs (UrbanSimPreprocessOutputs): Typed inputs generated by the preprocessor.
            launch_context (UrbanSimLaunchContext): Resolved mutable input and
                output paths for this invocation.
            model_run_hash (Optional[str]): An optional hash for the model run.
                If not provided, a new one will be created.

        Returns:
            UrbanSimRunOutputs: The typed outputs from the model run.
        """
        logger.info(
        ...
        settings = self.state.full_settings
        usim_datastore_fpath = launch_context.output_datastore
        ...
        # Collect outputs
        if usim_datastore_fpath.exists():
            logger.info(
    ...
    def run(
        ...
        return self._run(inputs, launch_context, model_run_hash)

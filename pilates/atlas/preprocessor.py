...
def copy_data_to_mutable_location(settings, output_dir):
    ...
    logger.info(
        "Copying atlas inputs from {0} to {1}".format(atlas_input_path, output_dir)
    )
    shutil.copytree(atlas_input_path, output_dir, dirs_exist_ok=True)
    if settings["start_year"] == 2017:
        logger.info(
            "Copying atlas warmstart outputs from {0} to {1}".format(
                atlas_output_path, output_dir
            )
        )
...
def prepare_atlas_inputs(settings, year, state: "WorkflowState", warm_start=False):
    ...
    urbansim_output_path = os.path.join(
    ...
    if year != state.year:
        ...
        for f in glob.glob(os.path.join(old_input_path, "*.RData")):
            if os.path.exists(os.path.join(atlas_input_path, Path(f).name)):
                ...
                logger.info(
                    "Moving file {0} to atlas input for year {1}".format(f, year)
                )
    ...
    with pd.HDFStore(urbansim_output, mode="r") as data:
        if not warm_start:
            try:
                ...
                logger.info(
                    "Preparing ATLAS Year {} Input from Urbansim Output".format(year)
                )

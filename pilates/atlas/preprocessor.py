    ...
    with pd.HDFStore(urbansim_output, mode='r') as data:
        if not warm_start:
            try:
                ...
                persons.to_csv('{}/graves.csv'.format(atlas_input_path))
...
def compute_accessibility(path_list, measure_list, settings, year, threshold=500):
    ...
    jobs_vector = jobs.groupby('block_id').agg({'job_id': 'size', 'zone_id': 'max'}).rename(
        columns={'job_id': 'access_sum'})
    ...
    taz_to_tract = pd.read_csv('{}/taz_to_tract_{}.csv'.format(settings['atlas_host_input_folder'], settings['region']),
                               index_col=0)
    ...
    accessibility_tract['access_zscore'] = (accessibility_tract['access_sum'] - accessibility_tract[
        'access_sum'].mean()) / accessibility_tract['access_sum'].std()

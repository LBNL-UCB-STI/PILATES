...
def build_beam_vehicles_input(settings, output_year, state):
    ...
    atlas_input_path = os.path.join(
        state.full_path, settings["atlas_host_mutable_input_folder"]
    )
    vehicles = pd.read_csv(
        os.path.join(atlas_output_path, "vehicles_{0}.csv".format(output_year)),
        dtype={"householdId": pd.Int64Dtype()},
    )
    mapping = pd.read_csv(
        os.path.join(
            atlas_input_path,
            "vehicle_type_mapping_{0}.csv".format(settings["atlas_scenario"]),
        )
    )
    mapping["numberOfVehiclesCreated"] = 0
    mapping.set_index(
        ["adopt_fuel", "bodytype", "modelyear", "vehicleTypeId"],
        inplace=True,
        drop=True,
    )
    ...
    allVehicles = []
    for (fuelType, bodyType, modelYear), vehiclesSub in vehicles.groupby(
        ["adopt_fuel", "adopt_veh", "modelyear"]
    ):
        try:
            ...
            try:
                ...
                bestOption = (
                    (temp.reset_index()["modelyear"] - modelYear).abs().idxmin()
                )
                ...
                try:
                    ...
                    try:
                        temp = mapping.loc[
                            (fuelType, slice(None), slice(None), slice(None)), :
                        ]
                        bestOption = (
                            (temp.reset_index()["modelyear"] - modelYear).abs().idxmin()
                        )
                        ...
                        try:
                            temp = mapping.loc[
                                (slice(None), bodyType, slice(None), slice(None)), :
                            ]
                            bestOption = (
                                (temp.reset_index()["modelyear"] - modelYear)
                                .abs()
                                .idxmin()
                            )
                            bestYear = temp.reset_index().iloc[bestOption, :][
                                "modelyear"
                            ]
                            matched = mapping.loc[(slice(None), bodyType, bestYear), :]
                        except KeyError:
                            bestOption = (
                                (mapping.reset_index()["modelyear"] - modelYear)
                                .abs()
                                .idxmin()
                            )
                            bestYear = mapping.reset_index().iloc[bestOption, :][
                                "modelyear"
                            ]
                            matched = mapping.loc[
                                (slice(None), slice(None), bestYear), :
                            ]
        createdVehicles = matched.sample(
            vehiclesSub.shape[0],
            replace=True,
        ...
        allCounts.loc[
            createdVehicleCounts.index, "numberOfVehiclesCreated"
        ] += createdVehicleCounts.values
        vehiclesSub["vehicleTypeId"] = createdVehicles.index.get_level_values(
            "vehicleTypeId"
        )
        vehiclesSub["stateOfCharge"] = np.nan
        allVehicles.append(vehiclesSub[["household_id", "vehicleTypeId"]])
    ...
    outputVehicles.to_csv(
        os.path.join(atlas_output_path, "vehicles_{0}.csv.gz".format(output_year))
    )
    allCounts.loc[allCounts.numberOfVehiclesCreated > 0, :].sort_values(
        by="numberOfVehiclesCreated", ascending=False

    ...
    def push_to_db(self, dbCon):
        try:
            if self.usim_record is not None:
                ...
                query = "insert into Household (household, {}) values (?, {});".format(
...
class Person:
    def __init__(self, id, per, from_usim=True):
        ...
        if from_usim:
            ...
            self.school_grade_level = max(
                min(per["student"] * (self.age - 3), 15), 0
            )  # use age to guess at grade level for those enrolled in school
            ...
            if "school_zone_id" in per:
                ...
                if pd.isnull(
                    per["school_zone_id"]
                ):  # if value exists but is null in urbansim data - replace with -1 due to not null constraint in polaris sqlite db
    ...
    def push_to_db(self, dbCon):
        if self.usim_record is not None:
            try:
                ...
                dbCon.execute(
                    ...
                    [
                        ...
                        self.marital_status,
                        self.race,
                        self.school_enrollment,
                        self.school_grade_level,
                        self.work_hours,
                        self.telecommute_level,
                        self.transit_pass,
                        self.work_zone_id,
                        self.school_zone_id,
                        self.time_in_job,
            ...
            try:
                fields_list = ",".join(self.data.keys())
                values_list = ",".join(["?"] * len(self.data.keys()))
                query = "insert into Person ({}) values ({});".format(
                    fields_list, values_list
                )
...
class Vehicle:
    ...
    def push_to_db(self, dbCon):
        try:
            fields_list = ",".join(self.data.keys())
            values_list = ",".join(["?"] * len(self.data.keys()))
            query = "insert into Vehicle ({}) values ({});".format(
                fields_list, values_list
            )
            dbCon.execute(query, list(self.data.values()))
        except sqlite3.IntegrityError:
    ...
    def Update_Zone_in_DB(self, DbCon):
        ...
        DbCon.execute(
            ...
            [
                self.households,
                self.persons,
                self.employment_total,
                self.employment_retail,
                self.employment_government,
                self.employment_manufacturing,
                self.employment_services,
                self.employment_industrial,
...
class Usim_Data:
    def __init__(self, forecast_year, usim_output):
        ...
        if not os.path.exists(usim_output):
            logger.critical(
                "Error: input urbansim data file path not found: " + usim_output
            )
            sys.exit()
...
def preprocess_usim_for_polaris(
    forecast_year,
    usim_output_dir,
    block_loc_file,
    db_supply,
    ...
    logger.info(
        "Starting polaris preprocessor for forecast year {0}".format(forecast_year)
    )
    ...
    if not os.path.exists(block_loc_file):
        logger.critical(
            "Error: census block to Polaris location mapping file path not found"
        )
        sys.exit()

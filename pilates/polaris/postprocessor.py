...
def generate_polaris_skims_for_usim(
    output_dir,
    database_name,
    NetworkDbPath,
    DemandDbPath,
    ...
    class OD_record:
        ...
        def calculate_gttime_factor(self, skim, wait_times):
            ...
            for i in range(0, len(skim.intervals)):
                ...
                l3_factor = self.get_factor(
                    vot_level, self.congestion_level_by_time[i], 1, 0, self.highway
                )
                l5_factor = self.get_factor(
                    vot_level, self.congestion_level_by_time[i], 0, 1, self.highway
                )
                tnc_factor = 1.0
                if tt > 0:
                    tnc_factor = (wait + tt) / tt
                # print ('*** ', tt, l3_factor, l5_factor, tnc_factor, wait)
                self.gttime_factor.append(
                    self.noAV
                    + self.l3 * l3_factor
                    + self.l5 * l5_factor
                    + self.tnc * tnc_factor
        ...
        for oid, o in OD_info.items():
            ...
            for did, d in o.OD_records.items():
                ...
                for i in range(0, len(skims.intervals)):
                    ...
                    skims.auto_ttime_skims[skims.intervals[i]][o_idx, d_idx] = (
                        tt * d.gttime_factor[i]
                    )
        print("Done.")
...
def update_usim_after_polaris(forecast_year, usim_output_dir, db_demand, usim_settings):
    ...
    if forecast_year:
        ...
        region_id = usim_get_setting(settings, "urbansim.region_mappings.region_to_region_id")[region]
    ...
    logger.info(
        ("Updating urbansim model, {0}, from polaris demand database...").format(
            usim_output
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-database_name",
        default="",
        help='Database name/filepath with no extension or schema indicator, i.e. "chicago", not "chicago-Supply" or chicago-Supply.sqlite',
    )
    parser.add_argument(
        "-vot_level",
        type=int,
        default=2,
        help="use 0 for VOTT low (from Scenario settings file - i.e. long run) and 1 for VOTT high (short run/less impact), 2 = no change",
    )
    ...
    vot_level = (
        args.vot_level if args.vot_level != 0 else 0
    )  # use 0 for VOTT low (from Scenario settings file - i.e. long run) and 1 for VOTT high (short run/less impact)

    postprocess_polaris_for_usim(
        args.database_name,
        NetworkDbPath,
        DemandDbPath,
        ResultDbPath,

...
def Main(
    ...
    origin_list=None,
    dest_list=None,
    limit_modes_list=None,
    limit_values_list=None,
):
    ...
    if origin_list is not None:
        skims = ReduceTransitSkims(skims, origin_list)

    if do_highway and write_bin:
        WriteHighwaySkimsV1(highway_skim_file, skims)
    if do_transit and write_bin:
        WriteTransitSkimsV1(transit_skim_file, skims)

    if do_highway and write_csv:
        WriteHighwaySkimsV1_CSV(
            highway_skim_file, skims, origin_list, dest_list, limit_values_list
        )
    if do_transit and write_csv:
        WriteTransitSkimsV1_CSV(
            transit_skim_file,
            skims,
            origin_list,
            dest_list,
    ...
    if do_highway and write_tab:
        WriteHighwaySkimsV1_TEXT(highway_skim_file, skims, origin_list, dest_list)
    # if do_transit and write_tab: WriteTransitSkimsV1_TEXT(ransit_skim_file, skims, origin_list, dest_list)

    if write_HDF5:
        WriteSkimsHDF5("test.hdf5", skims, do_transit)
...
def MEP(
    ...
    for kvp in skims.zone_id_to_index_map.items():
        ...
        mep_file.write(
            str(i) + "," + str(acost / adist) + "," + str(tcost / tdist) + "\n"
        )
...
def GetHighwaySkims(highway_skim_file, skims):
    ...
    for i in range(num_intervals):
        ...
        if data.size < size:
            print(
                "Error: matrix not read properly for interval "
                + str(i)
                + ", copying from previous"
            )
            skims.auto_ttime_skims[skims.intervals[i]] = skims.auto_ttime_skims[
                skims.intervals[i - 1]
            ]
        else:
        ...
        if version2:
            ...
            if data.size < size:
                ...
                skims.auto_distance_skims[skims.intervals[i]] = (
                    skims.auto_distance_skims[skims.intervals[i - 1]]
                )
            else:
                skims.auto_distance_skims[skims.intervals[i]] = data.reshape(
                    zones, zones
                )
            Check_Tag(infile, "EMAT", True)
            Check_Tag(infile, "BMAT", True)
            ...
            if data.size < size:
                ...
                skims.auto_cost_skims[skims.intervals[i]] = skims.auto_cost_skims[
                    skims.intervals[i - 1]
                ]
            else:
            ...
            if validate_against_highway:
                if id not in skims.zone_id_to_index_map.keys():
                    print(
                        "Error: zone id "
                        + str(id)
                        + " found in transit file that does not exist in highway file"
                    )
                else:
                    if skims.zone_id_to_index_map[id] != index:
                        print(
                            "Error: zone id has a different index in transit file from that found in highway file"
                        )
            else:
                if id not in skims.zone_id_to_index_map.keys():
                    skims.zone_id_to_index_map[id] = index
    ...
    for i in range(num_intervals):
        for mode in skims.transit_modes:
            ...
            if (mode == "BUS" and skims.bus_only) or not skims.bus_only:
                if data.size < tsize:
                    print(
                        "Error: transit ttime matrix not read properly: "
                        + str(data.size)
                        + ", "
                        + str(i)
                        + ", "
                    ...
                    skims.transit_ttime[mode][skims.transit_intervals[i]] = (
                        data.reshape(tzones, tzones)
                    )
            if version1:
                Check_Tag(infile, "EMAT", True)
            ...
            if (mode == "BUS" and skims.bus_only) or not skims.bus_only:
                if data.size < tsize:
                    print("Error: transit walk time matrix not read properly")
                else:
                    skims.transit_walk_access_time[mode][skims.transit_intervals[i]] = (
                        data.reshape(tzones, tzones)
                    )
            if version1:
                Check_Tag(infile, "EMAT", True)
            ...
            if (mode == "BUS" and skims.bus_only) or not skims.bus_only:
                if data.size < tsize:
                    print("Error: transit auto access time matrix not read properly")
                else:
                    skims.transit_auto_access_time[mode][skims.transit_intervals[i]] = (
                        data.reshape(tzones, tzones)
                    )
            if version1:
                Check_Tag(infile, "EMAT", True)
            ...
            if (mode == "BUS" and skims.bus_only) or not skims.bus_only:
                if data.size < tsize:
                    print("Error: transit_wait_time matrix not read properly")
                else:
                    skims.transit_wait_time[mode][skims.transit_intervals[i]] = (
                        data.reshape(tzones, tzones)
                    )
            if version1:
                Check_Tag(infile, "EMAT", True)
            ...
            if (mode == "BUS" and skims.bus_only) or not skims.bus_only:
                if data.size < tsize:
                    print("Error: transit_transfers matrix not read properly")
                else:
                    skims.transit_transfers[mode][skims.transit_intervals[i]] = (
                        data.reshape(tzones, tzones)
                    )
            if version1:
                Check_Tag(infile, "EMAT", True)
            ...
            if (mode == "BUS" and skims.bus_only) or not skims.bus_only:
                if data.size < tsize:
                    print("Error: transit_fare matrix not read properly")
                else:
                    skims.transit_fare[mode][skims.transit_intervals[i]] = data.reshape(
                        tzones, tzones
                    )
            if version1:
                Check_Tag(infile, "EMAT", True)
...
def ReduceTransitSkims(skim, zone_list):
    ...
    for zoneid in zone_list:
        if zoneid in skim.zone_id_to_index_map:
            ...
            print(
                "ERROR: "
                + str(zoneid)
                + " from reduced zone list is not in original list of zones."
            )
    ...
    for ozoneid in zone_list:
        for dzoneid in zone_list:
            if (
                ozoneid not in skim.zone_id_to_index_map
                or dzoneid not in skim.zone_id_to_index_map
                or ozoneid not in new_skim.zone_id_to_index_map
                or dzoneid not in new_skim.zone_id_to_index_map
            ):
                print(
                    "ERROR: "
                    + str(zoneid)
            ...
            new_skim.transit_ttime[o_idx, d_idx] = skim.transit_ttime[
                o_idx_orig, d_idx_orig
            ]
            new_skim.transit_walk_access_time[o_idx, d_idx] = (
                skim.transit_walk_access_time[o_idx_orig, d_idx_orig]
            )
            new_skim.transit_wait_time[o_idx, d_idx] = skim.transit_wait_time[
                o_idx_orig, d_idx_orig
            ]
            new_skim.transit_fare[o_idx, d_idx] = skim.transit_fare[
                o_idx_orig, d_idx_orig
            ]
            new_skim.auto_distance[o_idx, d_idx] = skim.auto_distance[
                o_idx_orig, d_idx_orig
            ]
...
def Check_Tag(file, check_val, exit=False):
    ...
    if tag.decode("utf-8") != check_val:
        if exit:
            print(
                "Error: " + check_val + " tag missing. Read as: " + tag.decode("utf-8")
            )
            sys.exit()
        else:
            print(
...
def WriteHighwaySkimsV1_CSV(
    ...
    with open(highway_skim_file + ".csv", "w") as outfile:
        ...
        if dest_list is None:
            dest_list = sorted(skims.zone_id_to_index_map.keys())
        ...
        for interval in sorted(skims.auto_ttime_skims.keys()):
            ...
            for i in origin_list:
                ...
                for j in dest_list:
                    ...
                    outfile.write(
                        str(skims.auto_ttime_skims[interval][i_id, j_id]) + ","
                    )
                outfile.write("\n")
            outfile.write("\n")
            ...
            if skims.version == 2:
                ...
                for i in origin_list:
                    ...
                    for j in dest_list:
                        ...
                        j_id = skims.zone_id_to_index_map[j]
                        outfile.write(
                            str(skims.auto_distance_skims[interval][i_id, j_id]) + ","
                        )
                    outfile.write("\n")
                outfile.write("\n")
                ...
                for i in origin_list:
                    ...
                    for j in dest_list:
                        ...
                        j_id = skims.zone_id_to_index_map[j]
                        outfile.write(
                            str(skims.auto_cost_skims[interval][i_id, j_id]) + ","
                        )
                    outfile.write("\n")
                outfile.write("\n")
        outfile.write("\n")
...
def ConvertTransitToV1(transit_skim_file, skims, zone_id_to_index):
    ...
    data = numpy.matrix(numpy.fromfile(infile, dtype="f", count=tsize))
    if data.size < tsize:
        print("Error: transit ttime matrix not read properly")
    else:
        skims.transit_ttime = data.reshape(tzones, tzones)

    data = numpy.matrix(numpy.fromfile(infile, dtype="f", count=tsize))
    if data.size < tsize:
        print("Error: transit walk time matrix not read properly")
    else:
        skims.transit_walk_access_time = data.reshape(tzones, tzones)

    data = numpy.matrix(numpy.fromfile(infile, dtype="f", count=tsize))
    if data.size < tsize:
        print("Error: auto distance matrix not read properly")
    else:
        skims.auto_distance = data.reshape(tzones, tzones)

    data = numpy.matrix(numpy.fromfile(infile, dtype="f", count=tsize))
    if data.size < tsize:
        print("Error: transit_wait_time matrix not read properly")
    else:
        skims.transit_wait_time = data.reshape(tzones, tzones)

    data = numpy.matrix(numpy.fromfile(infile, dtype="f", count=tsize))
    if data.size < tsize:
        print("Error: transit_fare matrix not read properly")
    else:
        skims.transit_fare = data.reshape(tzones, tzones)
...
def WriteTransitSkimsV1_CSV(
    ...
    for interval in sorted(skims.transit_intervals):
        ...
        for mode in skims.transit_modes:
            if limit_modes_list and mode not in limit_modes_list:
                continue
            ...
            if not limit_values_list or "IVTT" in limit_values_list:
                ...
                for i in origin_list:
                    ...
                    for j in dest_list:
                        ...
                        j_id = skims.zone_id_to_index_map[j]
                        outfile.write(
                            str(skims.transit_ttime[mode][interval][i_id, j_id]) + ","
                        )
                    outfile.write("\n")
                outfile.write("\n")
            ...
            if not limit_values_list or "WALK_OVTT" in limit_values_list:
                outfile.write(
                    mode + " Transit Walk Access Time Skim @" + str(interval) + "\n"
                )
                outfile.write(",")
                for i in dest_list:
                    outfile.write(str(i) + ",")
                ...
                for i in origin_list:
                    ...
                    for j in dest_list:
                        ...
                        outfile.write(
                            str(
                                skims.transit_walk_access_time[mode][interval][
                                    i_id, j_id
                                ]
                            )
                            + ","
            ...
            if not limit_values_list or "AUTO_OVTT" in limit_values_list:
                outfile.write(
                    mode + " Transit Auto Access Time Skim @" + str(interval) + "\n"
                )
                outfile.write(",")
                for i in dest_list:
                    outfile.write(str(i) + ",")
                ...
                for i in origin_list:
                    ...
                    for j in dest_list:
                        ...
                        outfile.write(
                            str(
                                skims.transit_auto_access_time[mode][interval][
                                    i_id, j_id
                                ]
                            )
                            + ","
            ...
            if not limit_values_list or "WAIT" in limit_values_list:
                ...
                for i in origin_list:
                    ...
                    for j in dest_list:
                        ...
                        j_id = skims.zone_id_to_index_map[j]
                        outfile.write(
                            str(skims.transit_wait_time[mode][interval][i_id, j_id])
                            + ","
                        )
                    outfile.write("\n")
                outfile.write("\n")
            ...
            if not limit_values_list or "TRANSFERS" in limit_values_list:
                ...
                for i in origin_list:
                    ...
                    for j in dest_list:
                        ...
                        j_id = skims.zone_id_to_index_map[j]
                        outfile.write(
                            str(skims.transit_transfers[mode][interval][i_id, j_id])
                            + ","
                        )
                    outfile.write("\n")
                outfile.write("\n")
            ...
            if not limit_values_list or "COST" in limit_values_list:
                ...
                for i in origin_list:
                    ...
                    for j in dest_list:
                        ...
                        j_id = skims.zone_id_to_index_map[j]
                        outfile.write(
                            str(skims.transit_fare[mode][interval][i_id, j_id]) + ","
                        )
                    outfile.write("\n")
                outfile.write("\n")
    ...
    for i in sorted(skims.intervals):
        ...
        if do_transit:
            for m in skims.transit_modes:
                ...
                tio = ti.create_dataset(
                    "walk_ovtt", data=skims.transit_walk_access_time[m][i]
                )
                tia = ti.create_dataset(
                    "auto_ovtt", data=skims.transit_auto_access_time[m][i]
                )
...
def ReadTransitSkims_CSV(
            ...
            for row in cr:
                if len(row) > 2:
                    if int(row[0]) not in skims.zone_id_to_index_map:
                        if row[0] not in ignored_zones:
                            ignored_zones.append(row[0])
                    elif int(row[1]) not in skims.zone_id_to_index_map:
                        if row[1] not in ignored_zones:
                            ignored_zones.append(row[1])
                    else:
    ...
    print(
        "Warning, the following tazs from skim .csv input were ignored as they are not in the skim_id_to_index_file:"
    )
    for id in ignored_zones:
        print(str(id))
            ...
            try:
                for row in cr:
                    ...
                    if read_count % (nzone * 50) == 0:
                        print(
                            str("Reading for interval {} is {:.2%} complete.").format(
                                interval,
                                float(read_count) / float(nzone) / float(nzone),
                            )
                        )
            except ValueError:
                print(row[2])
...
class Skim_Results:
    ...
    def print_OD_info(self, o_idx, d_idx, time, do_auto, do_transit, header=True):
        ...
        # print( i, o_idx, d_idx)
        if o_idx < len(self.zone_id_to_index_map) and d_idx < len(
            self.zone_id_to_index_map
        ):
            if header:
                s = "Time, Mode, IVTT, Walk_OVTT, Auto_OVTT, Wait_Time, Transfer, Fare\r\n"
            else:
                s = ""
            if do_auto:
                s += (
                    str(time)
                    + ", Auto,"
                    + str(self.auto_ttime_skims[self.intervals[i]][o_idx, d_idx])
                    + ","
                )
            else:
                s += "Auto, NA,"
            if do_transit:
                ...
                for mode in self.transit_modes:
                    if (mode == "BUS" and self.bus_only) or not self.bus_only:
                        s += (
                            mode
                            + ","
                            + str(self.transit_ttime[mode][interval][o_idx, d_idx])
                            + ","
                            + str(
                                self.transit_walk_access_time[mode][interval][
                                    o_idx, d_idx
                                ]
                            )
                            + ","
                            + str(
                                self.transit_auto_access_time[mode][interval][
                                    o_idx, d_idx
                                ]
                            )
                            + ","
                ...
                s += "TRANSIT,NA,NA,NA,NA,NA,NA"
            return s

    def print_skims(self):
        for interval in sorted(self.auto_ttime_skims.keys()):
            print("Skim end=" + str(interval))
            if len(self.auto_ttime_skims[interval]) > 0:
                print(self.auto_ttime_skims[interval])
            print("")

        print("transit_ttime")
        if self.transit_ttime.size > 0:
            print(self.transit_ttime)
        print("")
        print("transit_walk_access_time")
        if self.transit_walk_access_time.size > 0:
            print(self.transit_walk_access_time)
        print("")
        print("auto_distance")
        if self.auto_distance.size > 0:
            print(self.auto_distance)
        print("")
        print("transit_wait_time")
        if self.transit_wait_time.size > 0:
            print(self.transit_wait_time)
        print("")
        print("transit_fare")
        if self.transit_fare.size > 0:
            print(self.transit_fare)

    def resize_arrays(self, nzone):
        self.transit_ttime = numpy.resize(self.transit_ttime, (nzone, nzone))
        self.transit_walk_access_time = numpy.resize(
            self.transit_walk_access_time, (nzone, nzone)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-auto_skim_file",
        default="",
        help="An input auto mode skim file to read, in polaris .bin V0 or V1 format",
    )
    parser.add_argument(
        "-transit_skim_file",
        default="",
        help="An input transit mode skim file to read, in polaris .bin V0 or V1 format",
    )
    parser.add_argument(
        "-csv", action="store_const", const=1, help="Write CSV output flag"
    )
    parser.add_argument(
        "-tab", action="store_const", const=1, help="Write tab-delimited output flag"
    )
    parser.add_argument(
        "-bin", action="store_const", const=1, help="Write binary output flag"
    )
    parser.add_argument(
        "-hdf5", action="store_const", const=1, help="Write HDF5 output flag"
    )
    parser.add_argument(
        "-origin_list",
        type=int,
        nargs="*",
        help="A list of origin zone IDs used to generate a sub-skim file for only those zones",
    )
    parser.add_argument(
        "-destination_list",
        type=int,
        nargs="*",
        help="A list of destination zone IDs used to generate a sub-skim file for only those zones",
    )
    parser.add_argument(
        "-read_from_csv",
        action="store_true",
        help="Flag to indicate that skims will be read and created from  CSV input files, rather than from Polaris format. Automatically sets -bin and -csv to true. Requires -auto_skim_file and/or transit_skim_file to be provided as output",
    )
    parser.add_argument(
        "-convert_to_v1",
        action="store_true",
        help="Flag to indicate input skims are in V0 and need to convert to V1",
    )
    parser.add_argument(
        "-interactive",
        action="store_true",
        help="Flag to start interactive mode allowing for travel time requests between OD pairs",
    )
    parser.add_argument(
        "-batch",
        action="store_true",
        help="Flag to start batch mode allowing for travel time requests between multiple OD pairs, defined in trip_file",
    )
    parser.add_argument(
        "-trip_file",
        default="",
        help="CSV file of O,D,departure_times for use in batch mode.",
    )
    parser.add_argument(
        "-i1_transit_ttime_file",
        default="",
        help="Input transit skim csv file. One file describing the transit in-vehicle time for trip from O-D.",
    )
    parser.add_argument(
        "-i2_transit_walk_files",
        nargs="*",
        help="Input transit skim csv file. One or more files describing the walking time components for trip from O-D.",
    )
    parser.add_argument(
        "-i3_transit_wait_files",
        nargs="*",
        help="Input transit skim csv file. One or more files describing the waiting time components for trip from O-D.",
    )
    parser.add_argument(
        "-i4_transit_fare_file",
        default="",
        help="Input transit skim csv file. One ofile describing the fare for trip from O-D.",
    )
    parser.add_argument(
        "-i5_auto_distance_file",
        default="",
        help="Input transit skim csv file. One file describing the auto distance for trip from O-D.",
    )
    parser.add_argument(
        "-zone_id_to_index_file",
        default="",
        help="Map of zone ids to zone indexes. Required for skim file creation from csv files.",
    )
    parser.add_argument(
        "-i6_highway_ttime_files",
        nargs="*",
        help='Input csv file, required if read_from_csv to be true and no i*_transit files are specified.  One csv file for each skim time interval, with each row in "O,D,ttime" format.',
    )
    parser.add_argument(
        "-i7_highway_intervals",
        nargs="*",
        type=int,
        help="Defines the end times of the skim intervals for which the -i6_highway_ttime_files were created, one interval per file required.",
    )
    parser.add_argument(
        "-define_intervals",
        action="store_true",
        help="Flag to indicate that the intervals defined in auto_skim_fil will be altered to those defined in i7_highway_intervals.",
    )
    parser.add_argument(
        "-bus_only",
        action="store_true",
        help="flag to read only the bus+rail skims in order to save memory...",
    )
    parser.add_argument(
        "-limit_modes",
        nargs="*",
        help="Limit output to the specified modes - list of RAIL, BUS, PNR, PNRAIL",
    )
    parser.add_argument(
        "-limit_values",
        nargs="*",
        help="Limit output to the specified values - list of IVTT, WALK_OVTT, AUTO_OVTT, COST, TRANSFERS, WAIT, DISTANCE",
    )
    ...
    if args.tab == 1:
        write_tab = True
    if write_csv == False and write_tab == False and write_hdf5 == False:
        write_bin = True
    ...
    if args.read_from_csv:
        write_bin = True
        write_csv = True
        if args.zone_id_to_index_file == "":
            raise NameError("Error: missing zone_id_to_index file")

        if args.i6_highway_ttime_files is not None:
            ReadHighwaySkims_CSV(
                args.i6_highway_ttime_files,
                args.i7_highway_intervals,
                args.zone_id_to_index_file,
                skims,
            )
        ...
        if args.i1_transit_ttime_file != "":
            ReadTransitSkims_CSV(
                args.i1_transit_ttime_file,
                args.i2_transit_walk_files,
                args.i3_transit_wait_files,
                args.i4_transit_fare_file,
                args.i5_auto_distance_file,
                args.zone_id_to_index_file,
                skims,
            )
        ...
        if args.zone_id_to_index_file == "":
            raise NameError("Error: missing zone_id_to_index file")
        if args.auto_skim_file != "":
            raise NameError("Error: v0 to v1 converter not implemented for auto skims")
        if args.transit_skim_file != "":
            do_transit = ConvertTransitToV1(
                args.transit_skim_file, skims, args.zone_id_to_index_file
            )
            if do_transit:
                WriteTransitSkimsV1(args.transit_skim_file + "_v1", skims)
            if do_transit:
        ...
        while s != "q":
            ...
            if O_idx is None:
                raise NameError("Error: Origin zone id '" + O + "' not found.")
            if D_idx is None:
                raise NameError("Error: Destination zone id '" + D + "' not found.")

            if multiple_times:
                for t in skims.intervals:
                    print(
                        skims.print_OD_info(O_idx, D_idx, t, do_auto, do_transit, False)
                    )
            else:
        ...
        with open(args.trip_file[:-4] + "_out.csv", "wb") as outfile:
            outfile.write(
                "ID,O,D,Time,AutoTime,TransitTime,walk_time,wait_time,fare,dist\r\n"
            )
            with open(args.trip_file, "r") as infile:
                ...
                try:
                    for row in cr:
                        ...
                        if len(row) != 4:
                            print(
                                "Error: input must be in 'ID,O,D,departure_time' format."
                            )
                            continue
                        else:
                            id_offset = 1
                        ...
                        try:
                            ...
                            if O_idx is None:
                                raise NameError(
                                    "Error: Origin zone id '" + O + "' not found."
                                )
                            if D_idx is None:
                                raise NameError(
                                    "Error: Destination zone id '" + D + "' not found."
                                )
                            outfile.write(
                                ID
                                + ","
                                ...
                                + ","
                                + skims.print_OD_info(
                                    O_idx, D_idx, T, do_auto, do_transit, False
                                )
                                + "\r\n"
                            )
                        except ValueError:
                            print(
                                "Error: enter a valid OD pair seperated by a comma: "
                                + str(row)
                            )
                            continue

...
def run_polaris(forecast_year, usim_settings, warm_start=False):
    logger.info(
        "**** RUNNING POLARIS FOR FORECAST YEAR {0}, WARM START MODE = {1}".format(
            forecast_year, warm_start
        )
    )
    ...
    if warm_start:
        ...
        preprocessor.preprocess_usim_for_polaris(
            forecast_year,
            usim_output_dir,
            block_loc_file,
            db_supply,
    ...
    while loop < int(num_abm_runs):
        ...
        if loop == 0:
            if forecast_year:
                scenario_file = PR.update_scenario_file(
            ...
            PR.modify_scenario(
                scenario_file, "time_dependent_routing_weight_factor", 1.0
            )
            ...
            if warm_start:
                ...
                PR.modify_scenario(
                    scenario_file, "read_population_from_urbansim", "true"
                )
                ...
                PR.modify_scenario(
                    scenario_file, "output_moe_for_assignment_interval", "false"
                )
                PR.modify_scenario(
                    scenario_file, "output_link_moe_for_simulation_interval", "false"
                )
                PR.modify_scenario(
                    scenario_file, "output_link_moe_for_assignment_interval", "false"
                )
                PR.modify_scenario(
                    scenario_file,
                ...
                PR.modify_scenario(
                    scenario_file, "output_network_moe_for_simulation_interval", "false"
                )
                ...
                PR.modify_scenario(
                    scenario_file, "percent_to_synthesize", population_scale_factor
                )
                PR.modify_scenario(
                    scenario_file, "read_population_from_urbansim", "false"
                )
                ...
                PR.modify_scenario(
                    scenario_file, "output_link_moe_for_assignment_interval", "true"
                )
                PR.modify_scenario(
                    scenario_file,
                    "output_turn_movement_moe_for_assignment_interval",
                ...
                PR.modify_scenario(
                    scenario_file, "demand_reduction_factor", population_scale_factor
                )
                PR.modify_scenario(
                    scenario_file, "traffic_scale_factor", population_scale_factor
                )
            ...
            if forecast_year:
                scenario_file = PR.update_scenario_file(
                    scenario_main_file, forecast_year
                )
            PR.modify_scenario(
                scenario_file, "time_dependent_routing_weight_factor", 1.0 / int(loop)
            )
            PR.modify_scenario(scenario_file, "percent_to_synthesize", 1.0)
            PR.modify_scenario(scenario_file, "demand_reduction_factor", 1.0)
            PR.modify_scenario(
        ...
        PR.modify_scenario(
            scenario_file, "vehicle_distribution_file_name", veh_file_name
        )
        PR.modify_scenario(
            scenario_file, "fleet_vehicle_distribution_file_name", fleet_veh_file_name
        )
        ...
        # run executable
        success = PR.run_polaris_instance(
            data_dir, polaris_exe, scenario_file, num_threads, None
        )
        ...
        if success:
            ...
            PR.execute_sql_script(
                data_dir / demand_db_name,
                scripts_dir / "clean_db_after_abm_for_abm.sql",
            )
    ...
    if warm_start:
        postprocessor.update_usim_after_polaris(
            forecast_year, usim_output_dir, db_demand, usim_settings
        )
        ...
        archive_dir = postprocessor.archive_polaris_output(
            db_name, forecast_year, output_dir, data_dir
        )
        postprocessor.archive_and_generate_usim_skims(
            forecast_year, db_name, output_dir, vot_level
        )
        # only run the analysis script for the final iteration of the loop and process asynchronously to save time
        p1 = Thread(
            target=PR.execute_sql_script_with_attach,
            args=(
                archive_dir / demand_db_name,
                scripts_dir / "wtf_baseline_analysis.sql",
                archive_dir / supply_db_name,
            ),
        )
        p1.start()

...
def run_polaris(forecast_year, usim_settings, warm_start=False):
    ...
    if warm_start:
        ...
        preprocessor.preprocess_usim_for_polaris(forecast_year, usim_output_dir, block_loc_file, db_supply, db_demand,
                                                 1.0, usim_settings)
    ...
    if warm_start:
        ...
        p1 = Thread(target=PR.execute_sql_script_with_attach, args=(
        archive_dir / demand_db_name, scripts_dir / "wtf_baseline_analysis.sql", archive_dir / supply_db_name))
        p1.start()

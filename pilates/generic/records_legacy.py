...
class RecordStore:
    ...
    def to_mapping(self) -> Dict[str, str]:
        ...
        for record in self.all_records():
            key = record.short_name or record.unique_id
            if not key:
                logger.warning("Record missing short_name and unique_id; skipping.")
                continue

            path = None
            if getattr(record, "uri", None):
                path = record.uri
            elif isinstance(record, RepoRecord):
                path = record.repo_path
            elif hasattr(record, "file_path"):
